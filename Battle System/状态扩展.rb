...
# 【高级：状态的自动减少时机的扩展】
#
# - 在默认中，状态的自动减少时机仅有两种，分别为 行动结束时，回合结束时，
...
# 【设置：是否禁用默认状态】
#
#  若设置为 true ，则不再使用默认的状态，而都使用 Data_StateEX 状态对象。
#     对于 事件-附加状态，其 来源战斗者 为空，请不要在伤害设置中编写 a 。
#
#  若设置为 false ，则默认的状态和 Data_StateEX 各自存在，
...
#
#  2. 如果 没有设置层数 或设置了<层数 0>，则会作为新的一个状态附加，各自独立计数。
#     该特性允许了多个相同ID的状态独立计数，方便制作互相独立的中毒。
#
#------------------------------------------------------------------
# 【状态的附加属性】
#
...
# - 注意：
#
#  1. 如果状态的最大层数为0，即不可叠加，则每个独立状态都会触发一次伤害计算。
#     如果状态的最大层数为正数，则只会触发一次伤害计算，
#       请自己在伤害编写时利用 l 来增加状态层数对伤害值的影响。
#
#  2. 可以反复填写多次，以对不同类型进行设置。
#     对同一类型填写多次时，取最后一次的设置。
#
#  3. 状态的自动消除时机与该伤害计算为独立处理，
...
#     如果你有其他显示伤害方式需要兼容，请在 process_timing_formula 方法中编写。
#
#------------------------------------------------------------------
# 【高级：状态的自动减少时机的扩展】
#
...
module STATE_EX 
  ...
  def self.get_state_help(_state, _battler=nil) 
    ...
    if _battler
      ...
      t += get_state_help_text_count(type, v)
      # 受伤解除
