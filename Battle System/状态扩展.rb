...
# ● 增加功能一览
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
#  注：备注了【RGSS状态和状态对象】代表均生效，
#      备注了【仅状态对象】代表仅对本插件新增的状态对象生效。
#
# 【设置：是否禁用默认状态】
#
#  若设置为 true ，则不再使用默认的状态，仅使用 Data_StateEX 状态对象。
#     对于 事件-附加状态，其 来源战斗者 为空，请不要在伤害计算中编写 a 。
#
#  若设置为 false ，则默认的状态和 Data_StateEX 各自独立存在，
#     对于 事件-附加状态，其附加的依然是默认状态，无法进行本插件的伤害处理。
#
FLAG_NO_RGSS3_STATE = true 
#
# - 为了与默认状态独立，仅 state?(state_id) 修改为两类状态都检索。
#    battler.add_state    添加的是默认状态，
#    battler.add_state_ex 添加的是本插件的状态对象。
#
#---------------------------------------------------------------------
# △ 战斗者Battler新增方法一览                           【仅状态对象】
#---------------------------------------------------------------------
#
#  附加 1 个状态对象：
#
...
#
#    1) 同ID的状态共用一个回合计数，
#       使用 battler.remove_state(state_id) 将消除全部层数。
#       使用 battler.reduce_state_level(state_id, v) 能指定消除层数。
#
#  3. 对于状态对象（利用 add_state_ex 添加的状态）：
#
#    1) 如果设置了最大层数，则同一时刻只会有一个该状态存在，
#       且附加新的同ID状态时，不论层数是否增加，都将覆盖 battler_from、count、ps 变量。
#
#    2) 如果没有设置层数或设置了 <层数 0> ，
#       则附加新的同ID状态时，会另外添加一个新的状态，与已有状态各自独立计数。
#
#       该特性方便制作同状态ID但伤害来源不同的中毒。
#
...
#---------------------------------------------------------------------
#
# - 在默认中，状态的自动减少时机仅有两种，分别为 行动结束时 和 回合结束时，
#   而 受伤解除 和 战斗解除 都是无视计数直接消除状态，不属于自动减少。
#
# - 现在可以给状态设置全新的自动减少时机了。
#
# - 在 数据库-状态 的备注中填写：
...
#     <set timing 时机数字>
#
#   此时，数据库中设置的自动解除时机无效，但持续回合数会作为计数值。
#
#   然后，你可以在对应时机的脚本中调用战斗者的 update_state_turns_ex(时机数字) 方法，
#     将设置为该数字的全部状态进行 计数-1 处理，当计数为0时，将解除该状态。
#
# - 本插件已写好的状态自动减少的时机（“时机数字”替换为下列数字）：
#
#      1 → 角色行动结束时计数-1
#      2 → 战斗回合结束时计数-1
#      3 → 角色造成伤害后计数-1
#         （随 on_damage 方法计数，如果使用【技能多段伤害】，则会多次计数）
#      4 → 角色受到伤害后计数-1
#         （随 on_damage 方法计数，如果使用【技能多段伤害】，则会多次计数）
#
# - 注意：
#
#  1. 本插件中，对于当前行动/回合中附加的新状态，将不会处理 计数-1，
#     但如果你自己添加的时机在 on_action_end（其中清除了附加新状态的信息）后，
#     那么依然会执行 计数-1 。
#
#     因此，你可以先调用 battler.eagle_restore_added_states ，
#       将 on_action_end 执行前的“附加新状态信息”再读取出来，
#     之后再调用你的 update_state_turns_ex(数字) 方法，
#     最后调用 @result.added_states.clear 重置，
#       这样你添加的时机中也能让新附加状态不处理 计数-1 了。
#
#  2. 如果你自己有增加新的时机，那同样可以编写 
#       <timing 时机数字>...</timing> 来增加新时机的伤害处理。
#
#  3. 记得要自己在脚本的对应时机调用战斗者的 update_state_turns_ex(数字) 方法哦！
#     不然状态永远也不会自动消除了。
#
#---------------------------------------------------------------------
# △ 状态的自动减少与最大叠加层数的交互                  【仅状态对象】
#---------------------------------------------------------------------
# - 在默认系统中，状态的回合计数自动减少到 0 时，
#   将调用 erase_state(state_id) 消除全部同ID状态。
#   即不管目前有多少层，都会在回合计数归零时一起消除。
#
# - 虽然本插件的状态对象，在设置 <层数 0> 时能独立多次附加同一状态，
#   避免了没续上状态，导致层数全部失效而产生的挫败感。
#
#   但这样会重新显示一大堆同样的状态，不方便UI设计。
#
# - 对此，本插件针对新增的状态对象，添加了如下设置：
#
# - 在 数据库-状态 的备注中填写：
#
#     <层数抵扣消除>  或者  <reduce one level>
#
#   那么设置了最大层数的状态，如果已有层数大于1，
#   将在自动减少为0需要消除时，仅减少1层，而不消除全部层数。
#
#---------------------------------------------------------------------
# △ 状态的帮助文本                              【RGSS状态和状态对象】
#---------------------------------------------------------------------
...
#==============================================================================
class RPG::State
  attr_reader  :level, :timing_evals, :reserve_when_die, :reduce_one_level
  #--------------------------------------------------------------------------
  # ● 进入游戏时读取备注栏
  #--------------------------------------------------------------------------
  def reset_state_ex
