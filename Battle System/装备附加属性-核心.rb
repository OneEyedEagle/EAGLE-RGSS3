...
# - 本插件已为 $data_weapons 与 $data_armors 编写了绑定，
#     可以直接使用 $data_weapons[id_ex] 获取新增了附加属性的装备实例
#--------------------------------------------------------------------------
# ○ 操作流程
#--------------------------------------------------------------------------
# - 生成一个对指定装备item有效、内容为属性组attrs的附加属性的实例，
#   并返回它在附加属性实例数据库中的序号id_ex
#     id_ex = EQUIP_EX.new_ex(item, attrs)
#
# - 为指定装备item添加序号为id_ex的附加属性实例，
#   并返回绑定了该附加属性后的装备实例item_ex
#     item_ex = EQUIP_EX.get_equip(item, id_ex)
#
# -【可选/节约存储空间用】删去不再需要的含附加属性的装备，并删去对应附加属性实例
...
#     （若 value 值为 整数0 ，则该句属性调整会被自动删除）
#
# - 新增一个特性：
#     [code_id, data_id, value] → [编码ID, 数据ID, 值]
#   · 该特性为编辑器的数据库中右上角“特性”窗口的内容
#   · code_id 可以参照 Game_BattlerBase类开头的 常量（特性）列表中的值
#   · data_id 为编辑器中第一个下拉选择框的选项所对应的id
#     （部分从1开始，部分从0开始，推荐利用 p item.features 对照查看理解）
#   · value 为部分项所带有的第二个数值输入框的值
#     （若 value 值为 整数0 或 小数0.0，则会被自动删除）
#     （若数据库中没有，则推荐填入数字 1 ，并不产生任何效果，但可以防止被删除）
...
# ○ 可能问题
#--------------------------------------------------------------------------
# - 当需要进行装备实例的排序时，请调用其 id_o 方法获取装备在编辑器数据库中的 id
#--------------------------------------------------------------------------
# ○ 核心
#--------------------------------------------------------------------------
# - 对于每一个存档，为每一件装备新增了一个存储它专用的附加属性实例的hash
# - 将装备的id进行了扩展，变更为 数据库id_d+附加属性实例的id_ex
#   （如：数据库id为11的武器，其追加的附加属性实例id为23，则结合附加属性数量上限位数，
#         该自定义武器最后的显示id为11023）
...
module EQUIP_EX
  #--------------------------------------------------------------------------
  # ○ 常量：与存档文件相对应的存储装备附加属性实例数据库的文件
  #  其中 %d 将被替换成存档文件的ID号（从1开始）
  #--------------------------------------------------------------------------
  FILE_NAME = "Saves/Equip_EXs_%02d.rvdata2"
  #--------------------------------------------------------------------------
  # ○ 常量：每个装备的附加属性实例数据库的存储数目上限的位数
  #  如设置为 3 时，代表一件装备最多同时可以生成 999 个附加属性实例
  #--------------------------------------------------------------------------
  #（推荐：该位数 ≥ 游戏装备数量上限的位数，不然可能出现装备读取错位的BUG）
  EX_MAX_DIGIT = 3
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  # ○ 获取含有附加属性的装备的扩展序号
  #（若设置上限位数为3，当装备编辑器中id为2，附加属性id为1，则新装备id为2001）
  #  id_d：数据库中装备的序号id_d
