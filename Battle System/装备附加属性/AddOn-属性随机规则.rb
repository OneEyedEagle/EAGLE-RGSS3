...
#----------------------------------------------------------------------
# - 设置品质规则
#   （品质只会缩放附加的属性特性，而不会缩放VA数据库中装备的属性特性）
...
#
#   <exr lv lv_id {prob}> → 直接指定lv_id号品质的概率分子prob（eval字符串）
#     · prob 解析：在eval后返回的数字作为该lv_id号品质的概率分子，
#                  而概率分母为设置的全部品质的概率分子之和
#
#----------------------------------------------------------------------
# - 设置数目规则
#
#   <exr c max {value}> → 设置总共生成的随机属性有value条（最多有max条）
#                           （若未设置该项，将取通用设置）
#     · value 解析：被eval后返回的数字将作为该项的值
...
#
#    · sym 解析：Data_Equip_EX类中含有的相同名称的方法的字符串
#             （EQUIP_EX::PARAMS_TO_ID中设置的Key）
...
#
#    · value 解析：被eval后返回的值将作为该项的值（同上一部分中的value）
...
# ○ 应用规则
#--------------------------------------------------------------------------
# - 依据item备注栏中所设置规则，利用脚本生成一个随机的attrs数组
#
...
#      覆盖设置atk的因子为1、值为1+rand(2)，覆盖设置命中率因子为2、值为-0.2
...
# ○ 特别说明
#--------------------------------------------------------------------------
# - 由于并没做多余判定，传入的item只要是有note方法（数据库中有备注栏）的对象即可
#=============================================================================
module EQUIP_EX
  #--------------------------------------------------------------------------
  # ○ 常量：品质预设
