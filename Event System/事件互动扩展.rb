...
#
# - 特别的，本插件的互动不会锁定事件的方向，因此请手动添加【朝向玩家】
#
# 【高级】
#
# - 在【xx】文本中，增加 if{cond} 来设置该互动的出现条件
#
...
#
# - 事件会每帧判定目标事件是否在指定位置上，如果成功，则触发一次当前页里的对应互动
#   可以重复编写多次，以设置多个相互独立的自动触发
#
#   其中 auto 大小写任意
#   其中 tags 为以下语句的任意组合，用空格分隔，等号左右可以添加冗余空格
#
...
# 【注意】
#
# - 由于这个自动判定的触发同样是地图上玩家按键触发的模式，
#     即 触发时将挂起玩家操作，同时隐藏互动列表，
#     因此请不要编写太多高频率刷新的auto注释，那样将导致互动列表UI闪烁。
#
# - 如果实在需要不断刷新并触发的判定，推荐使用【事件消息机制】:
#     将注释中的 auto 改为 para （见下述【联动】），以调用事件消息进行并行处理，
#     该类型的刷新与触发不会挂起玩家操作，也不会出现互动列表闪烁。
#
#---------------------------------------------------------------------------
# 【联动】
#
...
#
# - 对于事件触发事件，由于利用玩家按键触发模式时，总会有一些奇怪的地方出现卡顿感，
#   因此同样新增了调用事件消息来进行处理：
#
...
#
#    就会改成用 并行执行 的方式调用本事件的【受贿】互动和小明事件的【贿赂】互动。
#
#   但注意，由于对话框是全局共有的，在执行对话时，依然会挂起全部操作。
#
#---------------------------------------------------------------------------
# 【兼容性】
#
...
class Game_Event < Game_Character
  ...
  def check_hash_trigger(_hash, type = :player)
    ...
    if _hash[:cond] && !EVENT_INTERACT.eagle_eval(_hash[:cond], self)
      return false
    end
