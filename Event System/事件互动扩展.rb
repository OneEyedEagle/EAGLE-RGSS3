...
# 【注意】
#
# - 具体调用效果同默认的 呼叫公共事件，当前事件将等待该调用结束
#
...
class Game_Interpreter
  ...
  def call_event(*args)
    ...
    if args.size == 1
      ...
      p "【警告】执行#{@map_id}号地图上#{@event_id}号事件中的事件脚本 call_event 时发现参数数目错误！"
      p " call_event 来自于脚本【事件互动扩展 by老鹰】，请仔细阅读脚本说明并进行修改！"
      return
    end
    ...
    if h[:pid] == nil || h[:pid] == 0
      ...
      page = event.event.pages[h[:pid]-1]
    end
    return if page == nil
...
class Game_Event < Game_Character
  attr_reader :event, :starting_type
  #--------------------------------------------------------------------------
  # ● 获取事件名称
  #--------------------------------------------------------------------------
  def name
