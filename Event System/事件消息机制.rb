...
#   若 cur_page 省略或传入 false，则在事件的全部事件页中搜索 LABEL 标签，
#               若传入 true，则只在当前激活的事件页中搜索 LABEL 标签
#
...
#
# 【注意】
#   1、为了保证事件的等待指令有意义，并且不出现明显的延时操作感，
#     当上一个消息未执行完，若再传入同一消息，将不会重复执行，并返回 true
#   2、为了确保不出现歧义，当事件存在消息执行时，自身的 start 将被阻止
#
# - 扩展：
