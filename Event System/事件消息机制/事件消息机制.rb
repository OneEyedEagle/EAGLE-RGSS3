...
# - 什么是事件消息？
#
#  → 消息是仿照QT Application中的触发器而设计的一种事件执行方式。
#    将事件看作一个对象，它有预设好的能独立执行的内容，每个内容有不同的名称，
#      当外部给它一个输入时，它会判定自己能不能响应这个输入：
#      即把这个输入和自己预设好的各执行内容的名称去比较，如果相同，就执行其内容。
#    这个输入-响应的过程就叫消息。
#    （在本插件中，统一使用字符串作为具体值）
#
#--------------------------------------------------
# - 为什么需要增加消息这一结构？
#
#  → 在默认RGSS中，如果想让一些内容能够同步执行，就要放到并行事件内，
#      但并行事件是一个完整独立的事件，这也就意味着它要占用地图上的格子或公共事件，
#      对于只想同步执行一次的内容，又要去占用开关、变量来全局控制并行事件的启用，
#      而且在编辑器中开启、关闭事件也非常繁琐，并行事件多了后更可能难以找到。
#    而利用事件消息，就能把内容直接写在当前事件页的最底部：
#      利用 中止事件执行 指令防止这些内容被正常的事件逻辑执行，
#      利用 事件脚本 触发当前事件的对应消息，轻松开启同步执行。
#
#--------------------------------------------------
# - 消息有什么优势？
#
#  → 并行执行，不占用额外事件：每个事件可以同时执行任意个不同名称的消息。
#
#--------------------------------------------------
# - 消息有什么不足？
#
#  → 由于本质是在当前地图上创建新的事件解释器，
#     当发生场景变化时，消息将被强制终止，没执行的内容都无法再继续了。
#
#--------------------------------------------------
# - 怎样在编辑器中设置消息？
#
...
#    |- 标签：END
#
#   这样就轻松设置了消息名为“这是消息名称”的一个消息：
#     利用 $game_map.events[1].msg("这是消息名称") 轻松触发它。
#
#   注意其中的 中止事件执行 指令，
#     这是为了确保正常执行该事件时，不会执行到消息的内容。
#
#-----------------------------------------------------------------------------
# ○ 使用消息：对于地图的事件（事件脚本中）
#-----------------------------------------------------------------------------
...
#
#     其中 ps 为扩展参数Hash，具体如下，可省略
#
#        :player => Game_Character类对象（比如 $game_map.events[1]）
#                  # 若设置，将用该事件对象替换消息中全部事件指令的“玩家”
#
#--------------------------------------------------
# 【示例】
#
...
#    |- 标签：END
#
#   msg("消息1")  → 并行执行 消息1 中的内容，继续执行之后的事件指令
#   msg("消息1", true)  → 等待 消息1 执行完成，再继续执行之后的事件指令
#   msg("消息2", false, 2) → 执行2号事件的 消息2 指令，同时继续执行之后的事件指令
#                            如果2号事件没有 消息2，则在控制台报错，游戏不中止
#   msg("消息2", true, 2, {:player => 1})  → 执行2号事件的 消息2 指令，等待完成
#                       2号指令的 消息2 中全部事件指令的“玩家”将被替换为1号事件
#
#--------------------------------------------------
# 【注意】
#
#    1. 该调用是直接另开一个并行处理以执行新的消息，
#         当 wait 传入 false 时，当前事件依然会继续执行。
#
#    2. 如果当前已经是在消息中了，则它的ps参数会拷贝一份。
#
#--------------------------------------------------
# 【消息中的便捷事件脚本】
#
...
# ○ 使用消息：对于地图的事件
#-----------------------------------------------------------------------------
# - 对于 Game_Event 对象，利用 msg(LABEL[, cur_page, ps]) 方法，并行调用指定消息
#
#    其中 LABEL 为事件中的标签内容的字符串
#               需要与 事件指令-标签 中所写的内容完全一致，包括前后空格
#
...
#
#     如果找到跳转的标签：
#       立即执行跳转后的消息内容，且丢弃之前消息的剩余指令。
#     如果未找到标签：
#       控制台输出报错，无视该跳转指令，继续执行之后的内容。
#
#   - 如果想在跳转后回到当前消息，请见【使用消息：对于地图的事件（事件脚本中）】。
#
#--------------------------------------------------
# 【特殊指令：自动触发的消息】
#
...
#     本插件添加了这样的实时判定：每帧更新前，判定是否发生跳转，
#       若成功，则会终止当前消息，并跳转到新消息，开始执行新消息的指令。
#
#   - 利用事件脚本编写一个实时判定：
#
...
#        与当前消息的搜索范围保持一致
#     其中 COND 是 eval 后返回 true 则进行跳转的脚本判定内容（字符串）
