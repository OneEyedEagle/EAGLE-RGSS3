...
#==============================================================================
$imported ||= {}
$imported["EAGLE-EventPositionMemory"] = "1.1.1"
#==============================================================================
# - 2025.6.9.0 修复存档无法保存位置的bug
#==============================================================================
# - 本插件新增对事件所在位置的存储
#--------------------------------------------------------------------------
# 【使用】
...
#   在进入地图/读档后，该事件页将会读取上一次离开地图时的所在位置。
#
# 【注意】
#
# - 事件的不同事件页所存储的位置是相互独立的。
#   即：离开地图时，事件的第二页为激活状态，则保存的位置为第二页所有，
#      当回到地图时，如果事件激活的为第三页，那么不会读取第二页的位置。
#
# - 离开当前地图时，全部事件的当前事件页的位置均会被保存。
#   进入新地图后，事件页第一个指令为注释，且含有特殊文本 <memory> 时，
#   才会读取之前离开地图时保存的位置。
#
# 【高级】
#
...
#    $game_system.load_event_pos(event)  → 读取已存储位置
#
#  对于某个事件的某一页，同时只会保存一个位置。
#
#==============================================================================
module EAGLE
  #--------------------------------------------------------------------------
...
if RUBY_VERSION[0..2] == "1.8"
  ...
  # ● 存档前的处理
  #--------------------------------------------------------------------------
  alias eagle_event_pos_on_before_save on_before_save
  def on_before_save
    eagle_event_pos_on_before_save
    $game_map.save_events_pos
  end
end
end
#==============================================================================
...
class Game_Map
  ...
  def setup(map_id)
    # 如果事件已经存在，且新地图ID不为当前地图ID，则发生了场所移动
    # 需要保存当前地图ID中的事件位置
    save_events_pos if @events && @map_id != map_id
  ...
  def setup_events
    ...
    load_events_pos # 在事件设置完成后，读取当前地图中之前保存的位置 
  end
  #--------------------------------------------------------------------------
  # ● 存储事件位置
  #--------------------------------------------------------------------------
...
class Scene_File
  ...
  # ● 执行存档
  #--------------------------------------------------------------------------
  alias eagle_event_pos_do_save do_save
  def do_save(*args)
    $game_map.save_events_pos
    eagle_event_pos_do_save(*args)
  end
end
end
