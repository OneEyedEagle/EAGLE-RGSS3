...
module MESSAGE_PARA
  ...
  #--------------------------------------------------------------------------
  FILE_MSG_LIST_NAMES = ["Data/PARA_MSG.txt"]
  #--------------------------------------------------------------------------
  # ●【常量】事件页注释里的并行对话类型
  # type_id => type_sym
  #--------------------------------------------------------------------------
...
module EAGLE
  ...
  #--------------------------------------------------------------------------
  class << self
    alias eagle_message_para_load_data load_data
    def load_data(filename)
      ...
      eagle_message_para_load_data(filename)
    end
  end
end
module MESSAGE_PARA
  ...
  def self.parse_list_file(filename)
    hash = {}
    text = EAGLE.load_data(filename) rescue ""
