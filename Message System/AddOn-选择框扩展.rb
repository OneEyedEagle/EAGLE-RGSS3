...
# - 在选择支内容中执行脚本：
#
#     rb{string} → 用 eval(string) 的结果进行替换
#
...
#     选项rb{v[2]+v[3]}内容ex{grb{v[1]}} → 在绘制前进行替换，变成：选项1内容ex{g200}
#------------------------------------------------------------------------------
# - 在选择支内容中新增条件扩展：
#
...
#          （只有最后一个有该设置的选项的倒计时生效）（无视禁止选择条件）
#     tv → 【重置】是否显示倒计时（1为显示，0为不显示）（默认显示）
#   （倒计时结束后禁止选择）
#     u → 【重置】设置该选择支在倒计时结束后禁止被选择（单位为帧）
#     uv → 【重置】是否显示倒计时（1为显示，0为不显示）（默认显示）
#   （倒计时结束后选项替换）
#     g → 【重置】设置该选择支在倒计时结束后自动变换成gi的目标选择支（单位为帧）
#          （该变换无视目标选择支的if{}条件）
#     gv → 【重置】是否显示倒计时（1为显示，0为不显示）（默认显示）
#     gi → 【重置】设置目标替换的选择支在事件列表的全部选择支中的序号
#          （从0开始计数，只看事件列表中的顺序，未被显示的也计算）
#          （取消分支的序号为事件列表的最后一个，中途的取消分支请全部忽略不计）
#
# - 示例：
