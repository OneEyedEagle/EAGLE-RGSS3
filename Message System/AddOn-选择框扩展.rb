...
#     w - 设置选择框的宽度（默认0不设置）（嵌入时该设置无效）（会自动保证显示完整文本的最小宽度）
...
module MESSAGE_EX
  ...
  # ● 获取指定转义符的基础设置
  #--------------------------------------------------------------------------
  class << self; alias eagle_choicelist_ex_get_default_params get_default_params; end
  def self.get_default_params(param_sym)
    return CHOICE_PARAMS_INIT if param_sym == :choice
    eagle_choicelist_ex_get_default_params(param_sym)
  end
end
#==============================================================================
# ○ Game_Message
