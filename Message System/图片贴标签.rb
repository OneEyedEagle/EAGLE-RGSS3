#==============================================================================
# ■ 图片贴标签 by 老鹰（https://github.com/OneEyedEagle/EAGLE-RGSS3）
...
#
#     TAG.new_sprite(id, s, ps={}) → 为精灵 s 创建空标签组，标签组名称为 id
#
#     TAG.new_map(id, x, y, ps={}) → 为地图格子(x,y)创建空标签组，名称为 id
#
#     TAG.new_map_chara(id, ps={}) → 为id号事件创建空标签组，名称为 id
#
#     TAG.new_pic(id, ps={})  → 为id号显示图片创建空标签组，其名称为 "pic-id"
#                                比如为 5 号图片创建后，它的标签组名称为 "pic-5"
#
#    其中 ps 为扩展参数，可添加以下设置：
#
...
#  - 标签组名称 id 需互不相同。
#
#  - 地图事件的标签组名称是数字，为避免冲突，自己新建的标签组推荐使用字符串作为名称。
#
#--------------------
# 2.1 利用全局脚本为指定id的标签组增加一个文本标签
#
#     TAG.add(id, tid, t, ps={}) 
#
#    其中 id 为标签组的名称。
#
#    其中 tid 为该文本标签的名称，之后删除标签时需要用到。
#
#    其中 t 为该文本标签的显示内容，字符串。
#
#    其中 ps 为扩展的参数，可添加以下设置：
#
#      :draw => {},  # 传入【组件-位图绘制转义符文本】中的参数组
#
# ※ 注意：
#
#  - 给同一标签组增加多个标签时，新加入的将放到预设位置，旧的往上平移。
#
#
#--------------------
# 2.2 利用全局脚本为指定id的标签组删除标签
#
#     TAG.delete(id, tid=nil) 
#
#       → 为名称为id的标签组，删去一个名称为tid的标签
...
# ※ 注意：
#
#  - 当所绑定的精灵被释放后，标签组将停止更新（但不会自动释放，请手动释放）。
#
#  - 绑定于地图事件的标签组，在地图切换时，将会自动释放。
#
#  - 在释放后，需要重新初始化标签组！
#
#-----------------------------------------------------------------------------
