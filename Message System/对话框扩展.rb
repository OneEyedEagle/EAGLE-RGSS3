...
#=============================================================================
# - 2020.11.12.23 新增env转义符，方便进行环境的设置
#=============================================================================
# 【兼容模式】
...
#    d → 对话框显示原点远离绑定对象中心点的像素值（按do自动选择增减xy）（默认0）
...
#    （若无变量参数的设置，可省略 | 符号）
#  - 若存在多个 \name ，只会取最后一个成功设置的名称，但变量参数会依次生效覆盖
#
# 【参数】
#    param → 变量参数字符串
# （变量一览）
...
#
# 【TODO：环境预设】
#  （此处为日后可能继续编写的部分，由于实现方式原因，暂未找到合适的环境初始化方式）
#  （请仍然选择在对话框中初始化指定环境，并在后续对话框调用该环境）
#     :font → 字体的变量哈希表
#     :win  → 对话框的变量哈希表
#     :pop  → pop对话框与tag的变量哈希表
#     :face → 脸图的变量哈希表
#     :name → 姓名框的变量哈希表
#     :pause → 等待按键的精灵的变量哈希表
#     :chara → 文字特效 { 文字特效转义符的符号 => 其对应变量哈希表 }
#     :ex → 扩展类转义符 { 扩展类转义符的符号 => 其对应参数字符串 }
#
#ENV_DEFAULT = {
#  # 环境名称 => { 预设 }
#  "1" => { :win => {:o => 2, :do => -2, :dy => -30, :w => 300, :h => 3 },
#         } # 对话框居下显示
#}
#
#----------------------------------------------------------------------------
