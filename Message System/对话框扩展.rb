...
#     将按照下面的排列顺序进行先后替换，因此请注意嵌套顺序
#
#  \conv[string] - 替换成 CONVERT_ESCAPE 中所设置的文本
#
#  \rb{string} - 【注意！该转义符参数用花括号】eval(string) 的结果将作为替换的文本
...
#    h - 设置窗口内容的固定高度（默认0不设置）（若小于line_height行高，则修正为行数）
#    dh - 设置窗口内容高度是否随文字绘制而动态变更（默认false）（不会自动翻页）
#    fh - 设置窗口内容高度是否直接指定为全部文字所需高度值（dh为true时有效）
#    hmin/hmax - 设置窗口内容的高度上下限（dh为true时生效）（若小于行高，则修正为行数）
#  （文本显示相关）
...
#    do - 相对于对话框的显示位置类型（对话框的九宫格位置）（0代表显示到文末）（默认2）
