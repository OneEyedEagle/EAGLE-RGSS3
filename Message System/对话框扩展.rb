...
#=============================================================================
# - 2020.10.21.21 优化姓名框更新；删去popt转义符；优化注释
#=============================================================================
# 【兼容模式】
...
#    ·“指令名”和“变量参数字符串”的大小写差异不会造成影响，但推荐全部用小写字母
#    ·转义符会在对话文本逐字显示到它时生效
...
# ● 扩展转义符列表
#=============================================================================
# - 以下将列举本插件新增加的各类扩展转义符，均可以直接写在对话框中进行使用
#
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# ○ 文本替换类
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
#----------------------------------------------------------------------------
#  \conv[string] → 替换成 CONVERT_ESCAPE 中所设置的 string 所对应的文本
#
# 【常量设置：文本替换】
# （由于解析问题，字符串中请将 "\" 替换成 "\\"）
...
#       测试对话：这是一句测试语句。
#----------------------------------------------------------------------------
#  \rb{string} → 替换成 eval(string) 的脚本执行结果（转化为字符串）
#
# 【注意】
...
#    name → 字体名称的索引号（在INDEX_TO_FONT中设置）（若为nil，则与默认一致）
#    size → 【默认】字体大小
#    i → 是否斜体（font.italic）
#    b → 是否加粗（font.bold）
...
#----------------------------------------------------------------------------
#  对话框脸图规格扩展：
#     脸图文件名包含 _数字1x数字2 时（其中为字母x），
#     将设置该脸图文件的规格为 行数（数字1）x列数（数字2）（默认2行x4列）
#  如： ace_actor_1x1.png → 该脸图规格为 1×1，含有一张脸图，只有index为0有效
#
#  \face[param] → 脸图的动态设置
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# ○ 文字特效类
#     以下转义符的 param 传入 任意非0非空字符（如 1）代表以预设值开启特效
#     只传入 0 代表关闭该特效
