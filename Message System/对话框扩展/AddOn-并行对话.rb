...
#==============================================================================
$imported ||= {}
$imported["EAGLE-MessagePara"] = "1.3.2"
#==============================================================================
# - 2024.11.22.17 兼容新版本对话框的forc_close
#==============================================================================
# - 本插件利用【对话框扩展】中的对话框，生成了并行显示的对话
#--------------------------------------------------------------------------
# ○ 概念：标签序列
#--------------------------------------------------------------------------
# - 由指定标签/标签对任意排列而组成的字符串，在脚本中将按顺序逐个解析标签并执行，
#   与默认事件页类似，但为了能使用字符串进行编写，而引入该结构。
#
# - 注意：
#    · 在实际解析前，将拷贝当前的 $game_message 作为该序列的对话框的初始状态
#    · 所有标签均大小写敏感，在默认情况下，需要使用小写字母
#
# - 可用标签一览：
#
#    <msg>...</msg>
#       → 设置对话文本
#          其中...替换成所需的对话文本
#          与【对话框扩展】中的转义符保持一致，但【不可】使用 \$ 转义符！
#       当解析到该标签对时，解析将暂停，直至对话框自动关闭
#       示例： <msg>这是一句测试用的文本，感觉没啥特别的哈！</msg>
#
#    <face>
#       → 重置脸图参数，之后的对话框变为无脸图
#
...
#       示例： <face actor1 0>
#                → 之后的对话框将显示 actor1 脸图文件中的第 1 个脸图（左上角第一个）
#
#    <msgp param_str>
#       → 设置对话框的相关参数
...
#                      若传入 数字，代表需要等待该帧数，再继续之后的指令
#                      若传入 0，代表需要等待动画播放结束，才继续之后的指令
#       示例： <anim -1 1 0>
#                → 玩家队伍中的1号角色显示1号动画，且等待动画结束，再继续之后的标签
#
#    <balloon chara_index balloon_id[ wait]>
#       → 显示心情气泡（仅在地图上时有效）
#         其中 balloon_id 为心情气泡的ID号（从1开始）
#       示例： <balloon 0 1>
#                → 当前事件显示1号气泡，不等待结束，直接继续之后的标签
#
#    <wait count>
...
#       示例：<until>s[1]</until>
#                → 等待，直至1号开关开启，再继续之后的标签
#
#    <if>str</if>
#       → 如果 eval(str) 返回 false，则跳过下一个标签
#       示例：<if>v[1] > 5</if>
#                → 如果1号变量的值小于等于5，则跳过下一个标签，继续执行之后的标签
#
#    <break>
#       → 直接结束当前序列
#
...
#
#--------------------------------------------------------------------------
# ○ 执行“标签序列”：征用 事件指令-滚动文本
#--------------------------------------------------------------------------
# - 当 S_ID_SCROLL_TEXT 对应的开关开启时，本插件将覆盖默认的滚动文本框指令
#
# - 滚动文本框的内容会被识别为 “标签序列”，且立即开始执行
#   同时我们约定：
#     第一行文本 为 唯一标识符name；
#     滚动文本框的禁止快进选项，将被识别为确保完全显示ensure_fin，
#       若勾选，则该并行对话序列必定显示完（场景切换后也会继续执行）
#
# - 注意，其中的转义符按正常方式进行编写
#
#--------------------------------------------------------------------------
...
#      <list {v[1]>0} f0w60>...</list> → 当1号变量大于0，自动循环显示，间隔60帧
#                                      且不会因为条件不满足而强制结束
#
# - 注意：
...
#
# - 打开 S_ID_NO_MSG 号开关时，会自动结束全部“标签序列”并禁止生成新的“标签序列”
#
#--------------------------------------------------------------------------
# ○ 高级：预设“标签序列” - 全局脚本
#--------------------------------------------------------------------------
...
#
#     event.distance_to(event_id)
#        获取 event 与第 event_id 号事件的距离|dx|+|dy|
#
