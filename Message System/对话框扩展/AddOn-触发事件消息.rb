#==============================================================================
# ■ Add-On 触发事件消息 by 老鹰（https://github.com/OneEyedEagle/EAGLE-RGSS3）
...
# - 注意：
#
#  · 由于在触发消息时，对话框还在等待中（并未关闭），
#     所以如果是默认系统，那消息中的显示文本指令就会卡住，无法继续！
#
#  · 可以使用【Add-On备用对话框 by老鹰】来生成新对话框，同步执行显示文本指令，
#     这样就可以继续在消息中使用显示文本了
#
#  · 可以使用类似于【并行式对话 by老鹰】或【聊天式对话 by老鹰】等新类型的对话方式，
#     来绕过默认系统中的显示文本及其 wait_for_message 等待，这样就不会卡住了
#
#----------------------------------------------------------------------------
# 【对话中途执行消息】
#
# - 转义符 \call[消息] 将在对话到它时直接执行对应的消息，同时继续显示对话
#
# - 转义符 \callw[消息] 将等消息执行结束，再继续后续对话
#
#==============================================================================
module MESSAGE_EX
  #--------------------------------------------------------------------------
...
class Window_EagleMessage
  ...
  # ● 在对话框执行中途触发消息
  #--------------------------------------------------------------------------
  def eagle_text_control_call(param = "")
    return if !@flag_draw
    e = $game_map.events[game_message.event_id]
    t = param
    e.msg(t)
  ...
  # ● 在对话框执行中途触发消息
  #--------------------------------------------------------------------------
  def eagle_text_control_callw(param = "")
    return if !@flag_draw
    e = $game_map.events[game_message.event_id]
    t = param
    e.msg(t)
    Fiber.yield
    Fiber.yield while e.msg?(t)
  end
end
#==============================================================================
# ■ 光标精灵
