...
===============================================================================
    ...
    └--------------------------------------------------------------------┘
     ...
     ╔  本脚本设计了一套利用转义符便捷激活各种动态效果的对话框，
     ╚  并且提供了多样化的组件，还能与默认对话框共同存在。
     ...
     ▼ 什么是转义符？ ·===================================================
      ...
      鼠标在编辑器显示文字的输入框中停留一段时间，即可看到默认支持的转义符，
        如 \| 代表文本显示到该位置时，将等待 1s 。
        如 \c[5] 代表之后的文本将使用 5 号颜色进行绘制。

      转义符即为特别约定的、能够起到特殊功能的文本。

     ╔ 在对话框中，我们约定转义符均以 \ 符号开头，之后接其他符号、字母、数字，
     ...
     ▼ 什么是扩展转义符？ ·===============================================
      ...
      本插件对默认转义符进行了扩展：
         在对话框中编写 \code[param] 类型的文本，
         即可执行 code 对应的功能，并将 param 作为参数传入。
      其中 \code[param] 就是本插件中所说的扩展转义符。

     ╔ \code 是什么？
     ╚    是本插件中已完成的各种功能，本插件中叫做“指令名”。

     ╔ param 是什么？
     ║    是特殊格式的字符串，本插件中叫做“参数串”。
     ║    由 字母+整数 或 字母+$ 的组合构成，
     ╚    可以增加空格和=符号，写成 字母=整数 或 字母=$，以方便阅读。

      在每一次利用扩展转义符激活某个功能时，你都可以写“参数串”设置。
       设置会被保留，所以不用每次都写一长串的重复设置。

      有的参数带有【重置】标记，说明每次激活，它都会重置为脚本中的默认值。
      有的参数带有【默认】标记，说明你可以不写它的名称，只写数字或$。
     ...
     □ 示例
      ...
      实际效果
        1. 在绘制到\foo转义符时，准备激活 foo 功能，“参数串”为 1 b=-1 tc=0 d$ 。 
        2. foo功能的变量 a 传入 1，变量 b 传入 -1，
           变量 tc 传入 0，变量 d 传入 nil 。
        3. 因为 foo 转义符的帮助中罗列的变量名称没有 tc 和 d ，
           虽然传入了它们，但不会起作用，也不会报错。

     ------------------------------------------------------------------
     □ 注意

      \code[param] 一般不在对话中显示，除非有特别说明。
     
      “指令名”和“参数串”的大小写差异不会造成影响。
     
      \code[param] 会在对话文本逐字绘制到它时生效，
        但若有【预先】标记，则会在全部文字绘制前生效（绘制途中不再生效），
        但若有【结尾】标记，则会在全部文字绘制后生效（绘制途中不生效）。
     ...
     □ 环境类
      ...
      \env    保存、读取指定的环境，方便切换不同的对话框设置
      \temp   对话框关闭时，重置为打开前的环境
...
#  ▼ 设置：兼容模式 ·=========================================================
#  
#   该设置用于兼容其他对话框，确保其他对话框正常使用。
#   若此常量被设为 true，则本对话框默认不启用，以其他对话框为主。
#    ╔ 此时，在事件脚本中调用 $game_message.eagle_message = true ，
#    ║   即可切换使用本对话框。
#    ╚ 而再调用 $game_message.eagle_message = false ，则可切回另一对话框。
...
# ┌-----------------------------------------------------------------
# ■ 按键判定：切换对话框显示/隐藏
# |- 该方法每帧判定，若返回 true 则会切换对话框的显示/隐藏。
...
# ┌-----------------------------------------------------------------
# ■ 按键判定：快进对话
# |- 该方法每帧判定，若返回 true 则会开启快进功能。
# |- 在对话框打开前会关闭快进功能，因此需要一直返回 true 才能保持快进状态。
# |- 如果使用了【Add-On 跳过对话】，则该设置无效。
  def self.force_close?
    Input.press?(:CTRL)
  end
#
# ◇ 常量：绘制N个文字后才更新对话框
# |- 正常情况下绘制一个文字后会更新一次对话框，
# |- 但快进状态下这样做必定出现卡顿，因此调整为绘制N个文字后更新一次。
# |- 对话框绘制完全部文字后必定更新一次。
  FORCE_CLOSE_N = 60
# └-----------------------------------------------------------------
...
# ■ 功能：内容滚动
# |- 当对话框的宽度或高度固定时，
# |  若文字超出对话框范围，将自动开启内容滚动功能。
# |- 此时，在 等待按键 状态下，按住 方向键 即可滚动浏览对话框全部内容。
#
# ◇ 常量：内容滚动时的动画帧数
# |- 当文字绘制超出对话框显示区域，导致内容需要滚动时，
#    新增了一个缓动动画来获得空行，该设置即为缓动动画的帧数。
  CHARAS_SCROLL_OUT_FRAME = 12
#
# ◇ 常量：内容无需滚动时，按方向键继续对话
# |- 当对话框内容无需滚动浏览时，
# |  若设置为 true，则使用方向键也可以继续对话，
#    若设置为 false，则仍然需要按确定键或取消键继续对话。
...
# |
# |- 下次对话框打开时，全部预定文本将插入到对话开头，之后清空预定文本。
# 
#   □ 注意
#
#   由于解析问题，文本中请将 \ 替换成 \\ 。
#   当存在多条预定文本，将按照时间顺序，依次放入对话开头。
#
# ◇ 示例：
...
# |- 利用特定转义符动态设置一些功能。
#
# ■ \func[param]  ▍【预先】在对话框打开前设置部分功能。
#
#  ◇ param“参数串”可设置变量一览
#
#    open    → 设置对话框使用 i 号打开方式
#    close   → 设置对话框使用 i 号关闭方式
#              i-0 默认的上下打开关闭
#              i-1 淡入淡出，
#              i-2 动态缩放，i-21 仅纵向缩放，i-22 仅横向缩放
#              i-3 动态滑入滑出
#
#    anim    → 是否启用缓动动画（包括打开、关闭、缩放等）
#
#    aw      → 是否开启对话框的自动换行（auto wrap）
#      ╔ 若开启，当文字绘制到对话框边界时，将自动换行。
#      ╚ 简单对超出部分换行，不考虑单词完整性、标点符号位于开头等情形。
#
#    para    → 【重置】是否并行处理下一个事件指令
#      ╔ 若开启，则对话框的下一条指令为 显示选项/数值输入/物品选择处理 时，
#      ╚ 会在对话框打开时，同步打开选择框/数字输入框/物品选择框，并关闭该功能。
#
#      □ 注意
#
#      并行开启时，选择框/数字输入框/物品选择框关闭时，对话框将同步强制关闭，
#      且选择框/数字输入框/物品选择框的嵌入对话框设置会失效！
#
#  ◇ 常量设置：默认值
FUNC_PARAMS_INIT = {
  :open => 2,   # 打开方式
  :close => 2,  # 关闭方式
  :anim => 1,   # 是否开启对话框打开、关闭等缓动动画
  :aw => 1,  # 是否自动换行
  :para => 0, # 是否并行处理子窗口
}
#
...
# ┌-----------------------------------------------------------------
# ■ 功能：缓动函数
# |- 本对话框在移动、大小变化等各个地方都使用了缓动函数。
# |- 此处可以对所用的缓动函数进行设置。
#
# ◇ 需要使用【组件-缓动函数 by老鹰】才能进行详细设置，否则默认基础函数。
#
# ◇ 获取缓动函数的返回值
  def self.ease_value(type, x)
    # type 为该函数被调用的场景
    # x 为 当前时间/总时间 的比值（0~1之间小数）
    ...
    return 1 - 2**(-10 * x)
    # return x  # 如果你不想使用缓动函数，直接返回 x 就是直线运动
  end
# └-----------------------------------------------------------------
...
#  ▼ 转义符：环境类 ·==========================================================
#
#   此处放置环境处理，效仿python的conda，将对话框的设置进行存储与读取。
# =============================================================================
...
# 
#   由于对话框可设置的变量众多，
#   当想要在不同情况使用不同样式的对话框时，不得不写一大串转义符设置，
#   而转义符的设置是全局的，想要用回之前的样式又要写一大串转义符，非常不便。
#   因此增加了该类别的转义符，一个环境便对应一套设置，以方便切换不同样式。
#
#   环境本质上是 Game_Message 类实例的拷贝。
# └-----------------------------------------------------------------

# ┌-----------------------------------------------------------------
# ■ \env[sym]  ▍【预先】读取并应用名称为 sym 的环境。
#
#  □ 注意
#  |- 默认环境为 0，因此可以用 \env[0] 切回最初的环境。
#  |- 当不存在名称为 sym 的环境时，将把当前对话框的环境存储为 sym 。
#
#  ◇ 示例
#  |- 对话编写
#       \env[测试]这是测试用对话。
...
#
#  ◇ 常量：预设环境
#  |- 环境的初始状态为各转义符的默认值，之后再依据此处设置进行覆盖。
#  |- 切换到此处的环境时，会先将环境重置为初始状态，即之前的改动无效。
#  |- 请将转义符的 \ 替换成 \\ 。
DEFAULT_ENVS = {
  # 环境名称 => "字符串"
  #"0" => "", # 默认环境名称为"0"，其中转义符均取默认值，此处写的环境"0"无效
  "底部" => "\\win[o2do-2dy-30w200h3]", # 对话框居下显示
}
#  ◇ 常量：当对话框开始绘制文本前，是否切换为环境0
#  |- 若设置为 true，则每次对话框绘制前，环境都将切换到环境 0 。
#  |- 若设置为 false ，则不会自动重置环境，请手动在对话里写 \env[0] 。
#  |- 若设置为 数字，则开关 $game_switches[id] 开启时，才自动重置环境。
  S_ID_RESET_ENV = true
# └-----------------------------------------------------------------

# ┌-----------------------------------------------------------------
# ■ \env[sym|save]  ▍【结尾】将当前对话框绘制完成时的环境保存为 sym ，
#                     同时当前所用的环境设置为 sym 。
#  ◇ 示例
#  |- 对话编写
...
# ┌-----------------------------------------------------------------
# ■ \temp  ▍【结尾】当前对话结束后，重置回对话框开启前的环境。
#
#  □ 注意
#  |- 在当前对话结束前，转义符的各种设置仍然生效。
#  |  因此中途生成的 并行对话，会继承当前对话中修改后的转义符设置。

#  □ 特别说明
#  |- 当对话文本中同时存在 \temp、\env[sym2]、\env[sym3|save] 时，
#  |  生效顺序如下（假设当前环境为 sym1）：
#     1. 对话框打开前，存储了一次 sym1 环境。
#     2. 执行【预先】转义符：
#          \temp 记录对话框打开前的环境为 sym1 ；
#          \env[sym2] 将读取并应用 sym2 环境。
#        此时环境为 sym2。
#     3. 对话框关闭后，
#          \env[sym3|save] 将对话框绘制完成后的环境存入 sym3，并应用 sym3 环境；
#          \temp 生效，将环境重置为 sym1 环境。
#     4. 继续下一个对话，此时环境为 sym1 。
# └-----------------------------------------------------------------
...
#   将按照脚本中从上到下的排列顺序进行替换，因此请注意相互之间的嵌套关系。
# =============================================================================

# ┌------------------------------------------------------------------
# ■ \conv[n] 或 \M[n]  ▍ 替换为脚本中预设的 n 号文本。
#
#  ◇ 参数释义
#
#    n   →   CONVERT_ESCAPE 中的键
#
#  ◇ 预设：替换文本
#  |- 由于解析问题，请将其中的 "\" 替换成 "\\"。
#  |- 如果有转义符用 <> 代替 []，如姓名框，则将它内部的 "\" 替换成 "\e"。
  CONVERT_ESCAPE = {
...
# ┌------------------------------------------------------------------
# ■ 设置：转义符别名
# |- 不需要写 \ ，脚本中会自动增加 \ 来查找并替换转义符。
# |- 将自动改为大写后进行替换。
# |- 为了避免歧义，此处只能替换带了 [] 的转义符。
  ALIAS_ESCAPE_CHARAS = {
    # 别名 => 目标转义符,
    #"bm" => "pop",  # 将 \bm[] 替换成 \pop[]
  }
...
# |- 为了避免歧义，此处只能替换带了 [] 的转义符。
  ALIAS_ESCAPE_CHARA_PARAM = {
    # 别名 => [转义符, 转义符中的某一个变量],
    "ali" => ["win", "ali"],  # 将 \ali[1] 替换成了 \win[ali1]
  }
# └------------------------------------------------------------------

# ┌------------------------------------------------------------------
# ■ \rb{string}  ▍ 替换为 eval(string) 的执行结果。
#
#  ◇ 参数释义
#
#    string → Ruby脚本语句，可以用 ; 分隔多句
#
#  ◇ 注意
...
# ┌------------------------------------------------------------------
# ■ \info[type+id,n]  ▍ 替换为数据库物品/武器/护甲的图标+名称。
#
#  ◇ 参数释义
#
...
#
#   为了确保win、pop等转义符能够顺利更改对话框属性，
#   位于文本开头的转义符将在对话框打开前优先处理完毕，然后再开始绘制文本。
#
# =============================================================================

# ┌------------------------------------------------------------------
# ■ \c[id]  ▍ 改变之后文字的颜色（扩展了默认的\c[id]）。
#
#  ◇ 参数释义
#
#    id → 可传入 TEXT_COLORS 常量中所设置的字符串，将匹配其对应的颜色，
#            如 \c[pink] 将使用 "pink" 所对应的颜色。
#         若匹配失败，将强制识别为数字，使用默认的索引颜色。
...
#            （若设置为nil，则为游戏默认字体）
#    size   → 【默认】字体大小
#    c      → 文字颜色的索引号（与 \c[id] 相同）
#        （若传入 -1，则重置为 DEFAULT_COLOR_INDEX ，不透明度重置为 255）
#    ca     → 文字的不透明度（0~255）
#    i      → 是否斜体（font.italic）
#    b      → 是否加粗（font.bold）
...
# ┌------------------------------------------------------------------
# ■ \win[param]  ▍ 设置对话框本身的各种参数。
#
#  ◇ param“参数串”可设置变量一览
#
#  （窗口属性相关）
#    z     → 对话框的Z值（仅正整数有效）
#    skin  → 对话框所用windowskin的序号
#    bg    → 对话框所用背景图片的序号
#          （只有当 事件指令-显示文字-窗口背景 为 普通窗口 时才生效）
...
#
#  （窗口位置相关）
#    o     → 【默认】对话框的显示原点的类型（对应九宫格小键盘）
#          （比如设置为 7，则以下位置均为对话框左上点的位置）
#    x/y   → 对话框显示原点在屏幕上的坐标（默认nil，取编辑器内设置）
#    do    → 将屏幕进行九宫格划分，对话框显示原点在屏幕上的位置（覆盖x/y的设置）
#          （有效值为-1~-9，-1在屏幕左下角，-5在屏幕中心点）
#    dx/dy → x、y坐标的额外增加量
#    fix   → 是否修正对话框位置，以保证对话框完整显示在屏幕内
#
#  （窗口大小相关）
#    w     → 窗口内容的宽度（不含边框）（默认0）
#           （若设置，则dw和fw均失效）
#    dw    → 窗口内容宽度是否随文字绘制而动态变化（默认0，不动态变化）
#    fw    → 窗口内容宽度是否为全部文字绘制完成时的宽度
#           （若设置为1，则dw失效）
#    wmin/wmax → 窗口内容宽度的最小值、最大值（启用dw/fw时才生效）
#    h     → 窗口内容的高度（不含边框）（默认0）
#           （若设置，则dh和fh均失效）
#           （若设置的数字小于行高，则识别为行数，将乘以字号作为高度）
#    dh    → 窗口内容高度是否随文字绘制而动态变化（默认0，不动态变化）
#    fh    → 窗口内容高度是否为全部文字绘制完成时的高度
#           （若设置为1，则dh失效）
#    hmin/hmax → 窗口内容高度的最小值、最大值（启用dh/fh时才生效）
#               （若设置的数字小于行高，则识别为行数，将乘以字号作为高度）
#
#  （文本显示相关）
#    se    → 打字音的序号
#    ali   → 文本对齐方式（0左对齐，1居中对齐，2右对齐）
#    ck    → 文字间距（默认0）
#    lh    → 行高（若为nil则取当前字号）
#    ld    → 行间距
#    cwi   → 文字显示后，下一个文字显示前的等待帧数（最小值0）
...
#
#  ◇ 常量设置：序号对应的窗口背景图片
#  （窗口背景图片的左上角会与对话框的左上角对齐）
#  （窗口背景图片存储于 Graphics/System 目录下）
  INDEX_TO_WINDOW_BG = {
    # 数字 => "背景图片名称"
  }
#
...
#  （音效文件存储于 Audio/SE 目录下）
  INDEX_TO_SE = {
    # 数字 => SE文件名, 音量, 音调
    0 => ["", 80, 100],  # 默认的无打字音设置，不推荐修改该项
    1 => ["Cursor1", 40, 150],
  }
#
#  ◇ 常量设置：播放一次打字音后，需要等待N帧才会再次播放，避免过于频繁
  CAHRA_SE_N = 10
#
#  ◇ 常量设置：播放打字音时，再根据字符使用不同打字音
  CAHRA_SE_ADJUST = {
    ['，', '。', '？', '！'] => 0,
  }
#
...
# ┌------------------------------------------------------------------
# ■ \move[param]  ▍ 立即移动对话框，并等待移动结束。
#    
#  ◇ param“参数串”可设置变量一览
#
#    t    → 移动所耗时间（默认20帧）
#    x    → 移动目的地x（若为0，则重置为win转义符中的x，或RGSS中初始位置0）
#    y    → 移动目的地y（若为0，则重置为win转义符中的y，或RGSS中初始位置底部）
#    dx   → 水平方向额外增加的移动距离（正数为向右，负数为向左）
#    dy   → 竖直方向额外增加的移动距离（正数为向下，负数为向上）
#
#  □ 注意
#
#  若设置了 \win[do=数字] 或启用了 \pop，则该移动指令无效。
#  对话框关闭时才会清除该转义符产生的位置偏移量。
#
#  ◇ 示例
#  |- 对话编写
#       这是测试对话，\move[dx=40 dy=30]看看到底怎么样。
#  |- 实际对话
#      （在显示完“这是测试对话，”后，对话框将在20帧内向右移动40向下移动30，
#        然后继续显示剩余的文字）
# └------------------------------------------------------------------
...
#  ◇ 参数释义
#
#    t    → 在t帧后自动结束按键等待，并继续事件处理
#          （默认nil，不会自动继续）
#
#  ◇ 常量设置：默认值
# （当未调用 \auto[t] 进行设置，或t为 nil 时，读取此处的值）
  WIN_AUTO_T = nil
#
#  ◇ 常量设置：自动播放UI中显示的文本
  WIN_AUTO_TEXT = "自动播放"
...
#
#  ◇ 常量设置：自动播放UI的显示原点
# （按九宫格小键盘，如 5 代表中心点为原点，9 代表右上角为原点）
  WIN_AUTO_O  = 9
#
#  ◇ 常量设置：自动播放UI的显示原点的所在位置
# （按九宫格小键盘，如 3 代表绑定到对话框的右下角，
#      5 代表绑定到对话框的中心点。）
# （特别的，负数代表绑定到屏幕上，如 -2 代表绑定到屏幕的底部中点，
#     -9 代表绑定到屏幕的右上角。）
  WIN_AUTO_DO = -9
#
#  ◇ 常量设置：自动播放UI的显示原点的左右、上下偏移量
  WIN_AUTO_DX = 0
  WIN_AUTO_DY = 0
#
#  ◇ 示例
...
# ┌------------------------------------------------------------------
# ■ \pop[param]  ▍ 【预先】启用气泡对话框。
#
#  ◇ param“参数串”可设置变量一览
#
#  （窗口属性相关）
#    z     → pop状态对话框的Z值（仅正整数有效）
#    skin  → pop状态对话框所用windowskin的序号（默认nil，取win转义符的skin的值）
#    bg    → pop状态对话框所用背景图片的序号
#          （只有当 事件指令-显示文字-窗口背景 为 普通窗口 时才生效）
#          （若图片读取成功，将不再显示窗口皮肤；若读取失败，仍绘制窗口皮肤）
#    bgo   → pop状态对话框背景图片与对话框的对齐原点（对应九宫格小键盘）
#          （默认背景图片的左上角与对话框左上角对齐，值为7）
#    bgx   → pop状态对话框背景图片的x坐标增加量
#    bgy   → pop状态对话框背景图片的y坐标增加量
#    skin  → pop状态下对话框所用皮肤的index
#
#  （窗口位置相关）
#    id    →【重置】【默认】气泡对话框所绑定的对象
#     【在地图上】0 取执行当前对话框的事件
#              正数 取当前地图id号的事件，目标事件不存在则取当前事件
#              负数 取队列中数据库id号的角色，目标角色不在队伍中则取队首
#     【在战斗中】正数 取敌群中序号为id（不是敌人数据库ID！）的敌人，不存在则无效
#                 负数 取我方参战角色中数据库id号的角色，不存在则无效
#      注 ╔ 默认RGSS战斗系统中，我方角色的xywh均为0，
#      意 ╚ 为避免对话框在左上角看不见，当绑定对象xy均为0时，气泡对话框设置无效。
#    mx/my →【重置】绑定到地图的 (mx, my) 格子底部中心处（编辑器坐标）
#           （若设置了 id，该设置无效）
#    o     → 气泡对话框的显示原点
#           （九宫格小键盘，如1代表左下顶点，5代表中心点，8代表顶部中点）
#           （若为nil，则为 10-do，即当do为8时，显示原点为底部中点2）
#    do    → 相对于所绑定对象，气泡对话框的显示原点的所在位置
#           （九宫格小键盘，如1代表目标左下角，5代表目标中心，9代表目标右上角）
#           （默认8，即对话框原点位于目标的顶部中间）
#    d     → 调整气泡对话框与绑定对象之间的距离（默认0，单位是像素）
#           （将根据 do 的设置，自动选择是增加还是减少x或y）
#    dx/dy → 在xy方向上的位置偏移量（默认0）
#    fix   → 是否修正气泡对话框位置，确保气泡对话框完整显示在屏幕内
#
#  （窗口大小相关）（与win的设置效果相同，但互相独立）
#    w     → 气泡对话框的内容宽度（默认0不设置）
#          （若设置，则dw/fw均失效）
#    h     → 气泡对话框的内容高度
#          （若设置，则dh/fh均失效）
#          （若小于行高，则高度为 h 乘以行高）
#    dw    → 是否随文字绘制而动态更新气泡对话框的宽度
#    fw    → 是否设为全部文字绘制完成时的宽度
#          （若设置为 1，则dw失效）
#    dh    → 是否随文字绘制而动态更新气泡对话框的高度
#    fh    → 是否设为全部文字绘制完成时的高度
#          （若设置为 1，则dh失效）
#
#  （指示箭头相关）
#    tag   → 所用的指示箭头的图片
#    td    → 指示箭头与绑定对象之间的距离（默认0）
#           （将根据 do 的设置，自动选择是增加还是减少x或y）
#
#  ◇ 常量设置：默认值
POP_PARAMS_INIT = {
  ...
  :td => 1, # tag与绑定事件格子中心位置的远离值
}
#  ◇ 常量设置：数字对应的指示箭头图片
#   （指示箭头图片存储于 Graphics/System 目录下）
  INDEX_TO_WINDOWTAG = {
...
#
#  ◇ 常量设置：窗口皮肤的序号 对应 指示箭头的序号
#  |- 方便给不同窗口皮肤设置对应的指示箭头图片
...
#
#  ◇ 常量设置：是否保证脸图显示在对话框内
#  |- 本插件允许脸图超出对话框范围，方便显示更大规格的脸图/立绘。
#  |- 但大家一般都在用默认 96x96 规格的脸图，且默认脸图能放在对话框里，
#  |    这时脸图在气泡对话框里会凸出一块，比较难看。
#  |- 因此增加该项设置：
...
#
#  □ 注意
#  |- 菜单等其他系统未做修改，依然只能识别默认规格的脸图文件。
#
#  ◇ 示例
#  |- ace_actor.png      → 规格2行4列，含8张脸图，与默认一致
#  |- ace_actor_1x1.png  → 规格1行1列，含1张脸图，index只有0有效
#  |- ace_actor_1x4.png  → 规格1行4列，含4张脸图
#
# └------------------------------------------------------------------

# ┌------------------------------------------------------------------
# ■ \facep[param]  ▍【预先】在对话框开启前设置脸图。
#
#  ◇ param“参数串”可设置变量一览
# 
#    dir   → 【默认】脸图的显示位置（0左侧，1右侧）
#    m     → 是否镜像显示脸图（默认0，不开启镜像）
#    it    → 脸图淡入时所用帧数
#    ot    → 脸图淡出时所用帧数
#    dx    → 脸图在x方向上的偏移量（默认0）
#    dy    → 脸图在y方向上的偏移量（默认0）
#    dw    → 当嵌入对话框内时，脸图宽度的增加量（默认0）
#    z     → 在对话框z值基础上，脸图的z值增加量（默认1，浮在对话框上面）
...
#  ◇ param“参数串”可设置变量一览
#
#    i     → 【重置】【默认】更改所显示脸图的序号为i
#    ls/le → 【重置】脸图自动循环播放的开始序号/结束序号
#           （若设置为 -1 ，则不启用自动播放）
#    lt    → 自动循环播放时，每两帧之间的等待帧数
#    lw    → 自动循环播放时，每一次loop结束时的等待帧数
#           （若设置为 nil ，则代表不循环）
#
# ├------------------------------------------------------------------
# ■ \facem[str|param]  ▍ 在绘制到该转义符时脸图执行指定动作。
#
#  ◇ 参数释义
#
#    str   → 脸图动作名称
...
#    move  → 脸图移动到指定位置
#
#      ◇ 动作move的param“参数串”可设置变量一览
#
#           x/y  → 移动目的地（以脸图移入后的位置为原点）
#         dx/dy  → 基于当前位置，朝x/y移动的距离
#             t  → 移动时间
#
#      ◇ 示例
#      |- \facem[move|dx=50]  →  脸图朝右移动50像素
#
#  □ 注意
#  |- 脸图动作期间，对话框文字将继续绘制，请调用 wait 转义符等待。
#  |- 若脸图动作时，又有新的动作，
#     则在当前动作执行完后，只会继续执行最后传入的一个动作。
#
#  ◇ 常量设置：默认值
FACE_PARAMS_INIT = {
# \facep[]
...
# ┌------------------------------------------------------------------
# ■ \name[姓名|param]  ▍【预先】设置姓名框。
#
#  ◇ 参数释义
#
#    姓名   → 显示的姓名
#             可以使用默认的绘制类转义符，但其中的[]要改为<>，避免歧义
#
#    □ 姓名的特殊设置
#    |- 如果只写数字，且在数据库中有该数字序号的角色，则将替换为该角色的名称
#    |- 示例
#         \name[1]   → 姓名显示为 艾里克
#         \name[999] → 因为数据库没有999号角色，姓名显示为 999
#
#    ◇ 常量设置：姓名前统一增加的字符串
#     （支持draw_text_ex中的转义符）（ \c[id] 支持 TEXT_COLORS 的扩展）
#     （由于解析问题，字符串中请将 \ 替换成 \e ，将 [] 替换成 <> ）
...
#
#       o    → 姓名框的显示原点
#             （按九宫格小键盘划分，默认 7 姓名框左上角为显示原点）
#      do    → 基于对话框的位置，姓名框显示原点的所在位置
#             （按九宫格划分，默认 7 位于对话框左上角）
#             （若设置为 0 ，则嵌入对话框，且 o 自动设为 7 ）
#      dx/dy → 姓名框x、y坐标的偏移量
#      opa   → 姓名框背景的不透明度（默认255）（不影响文字）
#      skin  → 姓名框所用皮肤（默认nil，同win中的窗口皮肤）
#      size  → 姓名框文字大小（默认nil，取Font默认值）
#      bg    → 姓名框背景图片的序号
#             （若读取成功，将不显示窗口皮肤，否则仍绘制窗口皮肤）
#      bgo   → 姓名框背景图片与姓名框的对齐点
#             （九宫格，默认7，即背景图片的左上角与姓名框左上角对齐）
#      cx/cy → 姓名框的文字起始绘制位置（默认为0,0）
#
#  □ 注意
#  |- 若对话中存在多个 \name 转义符，
#     只有最后一个设置的姓名生效，但“参数串”会依次生效并覆盖。
#
#  ◇ 常量设置：默认值
NAME_PARAMS_INIT = {
  :o => 7, # 自身的显示原点位置
...
#  ◇ 常量设置：姓名框自动调整位置，避免遮挡脸图
#  |- 由于姓名框可能遮挡脸图，本对话框新增该设置，确保不遮挡脸图。
#  |- 可利用脚本 $game_message.no_name_overlap_face = true/false 更改该设置。
...
# ┌------------------------------------------------------------------
# ■ \pause[param]  ▍ 设置等待按键时的动画。
#
#  ◇ param“参数串”可设置变量一览
#
#    id    → 【默认】使用的等待按键动画的序号
#            若设置为 -1 代表不显示；设置为 0 使用当前窗口皮肤。
#    o     → 帧动画的显示原点位置类型（按九宫格小键盘，默认 7 左上角）
#    do    → 基于对话框的位置，自身显示原点的所在位置
#           （按九宫格小键盘，默认RGSS中为 2 对话框底部中点）
#           （特别的，若设置为 0 ，则显示在文末）
#    dx/dy → x/y方向上的偏移值（默认0）
#    t     → 每两帧之间的等待帧数（默认10）
...
#  ◇ 常量设置：序号对应的等待按键动画
# （图片存储于 Graphics/System 目录下）
# （帧动画统一从左上开始为第0帧，之后先从左往右、再从上往下遍历）
  INDEX_TO_PAUSE = {
    #依次为：文件名，范围（nil则为整张图），一行中的帧数，一列中的帧数
    #index=>[String, Rect, Integer, Integer]
    -1 => ["", nil, 1, 1], # 不显示
    0 => [nil, Rect.new(96,64,32,32), 2, 2], # 窗口皮肤图片里的帧动画
  }
#  ◇ 常量设置：是否屏蔽默认的等待输入动画
# （指默认位于对话框底部中央的4帧动画，推荐屏蔽）
...
# ┌------------------------------------------------------------------
# ■ \shake[param]  ▍ 震动对话框并等待结束。
#
#  ◇ param“参数串”可设置变量一览
#
#    p     → 震动的强度（默认5）（同事件指令-震动画面）
#    s     → 震动的速度（默认5）
#    t     → 【默认】震动的持续帧数
#    y     → 是否由 x 方向的震动改为 y 方向的震动（默认0，即x方向）
#
#  ◇ 示例
#  |- 对话编写
...
# ┌------------------------------------------------------------------
# ■ \ins  ▍ 【预先】一次性绘制完全部文本，跳过全部等待。
# └------------------------------------------------------------------
#
# ┌------------------------------------------------------------------
# ■ \hold 或 \hold[str]  ▍ 【结尾】保留当前对话框。
#   
#  ◇ 参数释义
#
#    str   →【可选】任意文本，将作为该对话框的唯一标识符
#
#  □ 注意
#  |- 如果设置了str，
#  |    则新对话框打开时，将关闭已保留对话框中标识符为 str 的对话框。
#  |- 如果未设置str，
#  |    则该对话框始终保留，直到没有\hold指令的对话框关闭时，将一同关闭。
#   
#  ◇ 示例
#  |- 显示文本：这是测试对话。\hold[1]
...
#  |- 显示文本：这是第四局测试对话。
#  |- → 第一句对话结束时将不关闭，而是保留。
#     → 第二句对话结束时将不关闭，而是和第一句对话一起保留。
#     → 第三句对话打开前，先关闭有相同标识符1的第一句对话，
#                 结束时将不关闭，而是和第二句对话一起保留。
#     → 第四局对话结束时，第二句、第三句对话将和第四局对话一起关闭。
#
# └------------------------------------------------------------------

# ┌------------------------------------------------------------------
# ■ \close  ▍ 【结尾】当前对话结束后必定关闭对话框。
#             （哪怕之后连续的事件指令依然为显示文本）
# └------------------------------------------------------------------
#
# ┌------------------------------------------------------------------
# ■ \next  ▍ 【结尾】当前对话结束后不关闭对话框，
#             继续处理之后的事件指令，期间对话框将保持开启状态，
#             下一条显示文本指令，将在对话框中另起一行继续显示。
# └------------------------------------------------------------------
#
# ┌------------------------------------------------------------------
# ■ \clc  ▍ 对话框清屏。
#
#  □ 注意
#  |- 在绘制到该转义符时，对话框全部文字将向上移出显示范围，
#  |  然后在新的一行中继续绘制剩下的文字。
#  |- 在全部文字绘制完成后，你依然可以通过方向键滚动查看之前移出的文字。
#  |- 推进先设置好对话框的显示高度，再使用本功能，
#     否则在对话框高度动态变化的情况下，哪怕文字移出了，依然会计入文字高度，
#     导致底部出现较多的空白区域。
#
#  ◇ 常量设置：清屏的时间（单位：帧）
...
# ┌------------------------------------------------------------------
# ■ \cin[param]  ▍【独占】设置文字移入特效。
#                （移入完成后才处理其余文字特效）
#
#  ◇ param“参数串”可设置变量一览
#
#    r      → 是否将以下全部变量的值变更为正负范围内的随机数？
#             如设置 va10，则实际 va 为 -10 ~ 10 中随机整数。
#             如设置 t5，则实际 t 为 1 ~ 5 中随机整数（因为t最小值为1）。
#    t      → 移入所用帧数（最小1）
#            （不透明度从0平滑增加到255）
#    vzt/vz → 每 vzt帧（最小1）缩放值zoom 增加 vz（百分数）
#    va     → 每帧内角度值angle的增值
#    dx/dy  → 移入前所在位置的偏移量
#  （以下参数不受 r 的影响）
#    do     → 移入前文字所在位置
...
#            （设置 1~9 ，则为对话框的九宫格位置）
#            （设置 -1~-9，则为屏幕九宫格位置）
#             如：do5dx0dy0 就是文字从对话框中心移动到文本显示位置
#    vo     → 每帧不透明度的增量（设置 0 时将取值 255/t）
#    rxt/rx → 每rxt帧（最小1）src_rect的x增加rx（默认0）
#    ryt/ry → 每ryt帧（最小1）src_rect的y增加ry（默认0）
#
#  ◇ 常量设置：默认值
CIN_PARAMS_INIT = {
  :r => 0, # 随机取值？
...
#    t      → 移出所用帧数（最小1）
#           （不透明度从255平滑减小到0）
#    vzt/vz → 每 zvt 帧（最小1）缩放值zoom 增加 vz（百分数）
#    va     → 每帧内角度值angle的增值
#    dx/dy  → 移出后所在位置的偏移量
#  （以下参数不受 r 的影响）
#    do     → 移出后所在位置
...
# ┌------------------------------------------------------------------
# ■ \uout[param]  ▍【独占】利用消散移出文字。
#                 （移出时关闭其余特效更新）
#
#  ◇ 需前置 Unravel_Bitmap 插件才能使用该转义符。
#
#  ◇ param“参数串”可设置变量一览
#
#    n     → 消散的粒子总数（大约值）
...
#    o     → 单个粒子消失时的透明度变更最小值
#    dir   → 整体消散方向（同九宫格小键盘）
#           （1379-四角；5-四方向；46-左右向上）
#    s     → 粒子形状（0-正方向；1-圆形；2-三角形）
#
#  ◇ 常量设置：默认值
UOUT_PARAMS_INIT = {
# \uout[]
...
# ┌------------------------------------------------------------------
# ■ \cswing[param]  ▍ 开启文字左右摇摆特效（本质为精灵旋转）。
#
#  ◇ param“参数串”可设置变量一览
#
#    d     → 每次增加的角度值
#           （若设置 0 ，则角度直接到最大值，但初始摆动方向随机）
#    t     → 每次角度更新后等待t帧
#    t2    → 每次角度到达最大值后等待t2帧
#    a     → 角度可到达的最大值（左右对称）
#    o     → 设置摇摆的不动点（九宫格，5为文字中心点，2为文字底部中点）
#
#  ◇ 常量设置：默认值
CSWING_PARAMS_INIT = {
  :d => 0, # 每次更新增加的角度值
...
#    o     → 设置缩放的不动点（九宫格，5为文字中心点，2为文字底部中点）
#    min   → x和y方向上缩放总量的最小值
...
# ┌------------------------------------------------------------------
# ■ \cshake[param]  ▍ 开启文字抖动特效。
#
#  ◇ param“参数串”可设置变量一览
#
#    l/r/u/d  → 设置 左右上下 四个方向的最大偏移值
#    vx/vy    → 设置x、y方向上的初始移动速度（正数为向右、向下）
#              （若设置 0，则为随机方向）
...
# ┌------------------------------------------------------------------
# ■ \cshake2[param]  ▍ 开启文字抖动特效（频率更大的震动）。
#
#  ◇ param“参数串”可设置变量一览
#
#    dx     → 往左或往右的最大震动距离
...
# ┌------------------------------------------------------------------
# ■ \cflash[param]  ▍【叠加】开启文字闪烁特效（本质为精灵flash）。
#
#  ◇ param“参数串”可设置变量一览
#
...
# ┌------------------------------------------------------------------
# ■ \cmirror[param]  ▍【叠加】开启文字X方向镜像（本质为精灵mirror）。
#
#  ◇ param“参数串”可设置变量一览
#
#    无参数，传入 1 即可启用。
...
# ┌------------------------------------------------------------------
# ■ \ctog[param]  ▍【叠加】开启文字切换特效（本质为切换精灵的bitmap）。
#
#  ◇ param“参数串”可设置变量一览
#
#    i     → 使用 i 号文字组
#    n     → 从文字组中随机挑选 n 个字符作为切换文字（若为0，则取全部）
#    t     → 文字切换一次后的等待帧数
#    r     → 是否启用随机切换（若设置 0，则按顺序切换）
#
#  ◇ 常量设置：默认值
CTOG_PARAMS_INIT = {
  :i => 0,  # 选取i号文字组
...
# ┌------------------------------------------------------------------
# ■ \cneon[param]  ▍【叠加】开启文字霓虹变色特效（本质为精灵color变换）。
#
#  ◇ param“参数串”可设置变量一览
#
#    t     → 颜色之间的渐变帧数
...
# ┌------------------------------------------------------------------
# ■ \cmc[param]  ▍【叠加】开启文字叠加绘制特效。
#
#  ◇ param“参数串”可设置变量一览
#
#    i     → 使用 i 号文字组
#    n     → 从文字组中随机挑选 n 个字符作为叠加文字（若为0，则取全部）
#    c     → 叠加文字的颜色索引号（-1时与原始文字颜色一致）
#
#  ◇ 常量设置：默认值
CMC_PARAMS_INIT = {
  :i => 0,  # 选取i号文字组
...
#  ◇ 常量设置：序号对应的文字组
CMC_CHARAS = {
  # 定义文字叠加绘制的文字组（若填写数字，则取 IconSet 中的图标）
  0 => ['✖'],
  1 => [4],
}
# └------------------------------------------------------------------
...
# ┌------------------------------------------------------------------
# ■ \cfk[param]  ▍ 开启文字明灭闪烁特效。
#
#  ◇ param“参数串”可设置变量一览
#
#    t     → 明灭的持续帧数（不透明度从255到0的所用时间）
#           （设置 0 则直接显隐，设置 负数 代表 0~|负数| 间的随机数）
#    w     → 下一次明灭前的等待帧数（nil时不再明灭）
#    h     → 若设置 1，则会在不透明度为 255 时为结束一次循环
#            若设置 0，则会在不透明度为 0 时为结束一次循环
#
#  ◇ 常量设置：默认值
CFK_PARAMS_INIT = {
  :t => 30, 
...
#  ▼ 转义符：脚本类 ·==========================================================
#  
#   此类别的转义符需要一定的脚本知识才能灵活运用。
# =============================================================================

# ┌------------------------------------------------------------------
# ■ \eval{string} 或 {{string}}  ▍ 当绘制到该转义符时，执行 eval(string)。
#                                  eval(string) 的返回值将被丢弃。
#  ◇ 参数释义
#
#    string    → Ruby语句的字符串，可以用 ; 分隔多句脚本
#
#  □ 注意
#  |- 该转义符使用花括号，同时 string 中不可出现花括号。
#
#  |- 可用 s 代替 $game_switches ，用 v 代替 $game_variables
#     可用 msg 代表当前对话（Window_Message类的实例）
#     可用 last_chara 获取上一个文字精灵（但注意，可能会被cin覆盖设置！）
#
...
#  □ 注意
#  |- 单个文字可以被存入多个分组中。
#  |- 全部文字均在 0 分组内，因此当 sym=0 时表示取消分组。
#
#  □ 高级
#
...
#
# ├------------------------------------------------------------------
# ■ \setm[sym|effect|param]  ▍ 对 sym 分组内的全部文字启用文字特效。
#  
#  ◇ 参数释义
#
#    sym      → 分组名称
#    effect   → 文字特效类的转义符的名称，如 ctog
#    param    → 对应转义符的param“参数串”，具体查看各转义符的帮助文本，
#               若只写 0，则为关闭该文字特效
#
#  ◇ 示例
#  |- 对话编写
...
# ■ \cg[c1..]  ▍ 文字开启纵轴渐变色描绘。
#
#  ◇ 需前置 Sion_GradientText 插件才能使用该转义符。
#
#  ◇ 参数释义
#
...
class Window_EagleMessage < Window_Base
  ...
  def eagle_process_after_check_continue
    if @flag_hold  # 保留当前对话框时
      # 如果想要对话框从当前位置继续移动，则需要该处理，否则是在新位置打开
      #self.openness = 255 
      #@flag_need_open = false
    end
    eagle_message_reset_continue
