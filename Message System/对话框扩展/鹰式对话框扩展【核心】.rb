...
#==============================================================================
  ...
  #  k → 字符间距  ld → 行间距
  def self.calculate_text_wh(bitmap, text, k=0, ld=0, font_params=nil)
    apply_font_params(bitmap.font, font_params) if font_params
  ...
  # 将 font_params 应用到 Font 对象上
  def self.apply_font_params(font, ps) # ps = font_params
  ...
  # 设置指定对象的显示原点
  # （由于一些对象无法设置显示原点，故用该方法更改对象的 x 和 y，以替代更改显示原点）
...
class Game_Message
  ...
  #--------------------------------------------------------------------------
  # ● 重置因为子窗口而额外增加的对话框宽高（new_page时调用）
  #--------------------------------------------------------------------------
...
class Window_EagleMessage < Window_Base
  ...
  def eagle_update_assets_after_open
    eagle_update_xy if game_message.pop?
    @eagle_window_name.update
  ...
  def eagle_open_type_fade
    ...
    while ( _opa < 255 )
      ...
      Fiber.yield
      eagle_update_xy # 因为update中不更新对话框位置，故加入一次更新
    end
  end
  ...
  def eagle_close_type_slide
    ...
    @flag_no_fix = true
    # 直接指定目的位置在屏幕外
    h = {:ease_type => :msg_close_slide, #:close => true, # 不作为关闭处理，避免背景缩小为0
  ...
  def eagle_close_type_fade
    ...
    while ( _opa > 0 )
      ...
      update_assets_opacity(_opa)
      Fiber.yield
      eagle_update_xy # 因为update中不更新对话框位置，故更新一次
    end
    self.openness = 0
    close
    @eagle_offset_y = 0
  ...
  def eagle_set_params_xywh(_p = {})
    ...
    _p[:t] ||= MESSAGE_EX::MSG_WH_CHANGE_FRAME
  ...
  # wh变化后的处理
  # （此时 self.width 与 self.height 为实时宽高，不是最终宽高）
  def eagle_after_wh_change
    eagle_update_xy  # 先更新对话框位置
  ...
  def update_back_sprite_zoom(max_w = nil, max_h = nil, _p = {})
    ...
    if max_w == nil # 如果最大宽度为 nil，则说明不需要动态变化背景图片
      ...
      if _p[:i] and _p[:i] == 0 # 跟随宽高动态缩放
        ...
        # 再除以图片的宽度，作为初始缩放值
        _p[:bg_zx0] = [@back_sprite.width, self.width, max_w].min * 1.0 / @back_sprite.width
        _p[:bg_zx1] = _p[:close] ? 0 : 1  # 如果是关闭，则最终缩放为0
        _p[:bg_zxd] = _p[:bg_zx1] - _p[:bg_zx0]
    ...
    if max_h == nil
      ...
      if _p[:i] and _p[:i] == 0
        _p[:bg_zy0] = [@back_sprite.height, self.height, max_h].min * 1.0 / @back_sprite.height
        _p[:bg_zy1] = _p[:close] ? 0 : 1
        _p[:bg_zyd] = _p[:bg_zy1] - _p[:bg_zy0]
  ...
  #--------------------------------------------------------------------------
  # ● 窗口的实际xy（初始化/一页绘制完成时调用）
  #--------------------------------------------------------------------------
  def eagle_update_xy
    if game_message.pop? && @eagle_pop_obj
      params = pop_params
      eagle_pop_update
      eagle_pop_xy_ex
    else
    ...
    self.x = self.x + params[:dx] + @eagle_move_x + @eagle_offset_x
    self.y = self.y + params[:dy] + @eagle_move_y + @eagle_offset_y
    eagle_fix_position if params[:fix]
    eagle_after_update_xy
    eagle_change_windowskin(params[:skin])
  end
  
  # （扩展用）定好对话框的初始位置后，还可以再进行额外的修改
  def eagle_win_xy_ex
  end
  def eagle_pop_xy_ex
  end
  ...
  def new_page(text, pos)
    ...
    pos[:x] = new_page_chara_x; pos[:y] = new_page_chara_y  # 重置pos参数
    pos[:new_x] = new_page_chara_x; pos[:height] = 0
  ...
  # 文字绘制时的初始位置
  def new_page_chara_x;  0;  end
  def new_page_chara_y;  0;  end
  ...
  # 文字移入移出时，由于解绑了视图，靠此方法强制更新其存储的显示区域的显示原点
  ...
  def recreate_contents_for_charas
    ...
    w = [eagle_charas_max_w, w].min if MESSAGE_EX::WINDOW_MAX_WIDTH > 0
  ...
  def eagle_new_chara_sprite
    ...
    s = MESSAGE_EX.charapool_new(self, f, @eagle_chara_viewport)
  ...
  def process_normal_character(c, pos)
    ...
    eagle_process_draw_before(bitmap_width, bitmap_height, pos)
    if @flag_draw
  ...
  def process_draw_pic(text, pos)
    ...
    h[:h] ||= _bitmap.height
    h[:opa] ||= 255
    eagle_process_draw_before(h[:w], h[:h], pos)
    if @flag_draw
      s = eagle_new_chara_sprite
      s.reset_origin_xy(pos[:x], pos[:y])
  ...
  # ● 生成文字精灵前的处理
  #--------------------------------------------------------------------------
  def eagle_process_draw_before(c_w, c_h, pos)
  ...
  # 原本 Window_Message 的控制符的处理
  #  返回 true 则代表转义符执行成功，否则需要继续匹配新增的其它转义符
  def eagle_call_process_escape(code, text, pos)
    handler = ESCAPE_HANDLERS[code.upcase]
    if handler and respond_to?(handler)
      send(handler, text, pos)
      return true
    end
    return false
  end
  ...
  ESCAPE_HANDLERS = {
    '$' => :handle_escape_gold_toggle,
    '.' => :handle_escape_wait_15,
    '|' => :handle_escape_wait_60,
    '!' => :handle_escape_pause,
    '>' => :handle_escape_activate_show_fast,
    '<' => :handle_escape_deactivate_show_fast,
    '^' => :handle_escape_skip_pause,
    'C' => :handle_escape_change_color,
    'PIC' => :handle_escpae_draw_pic,
  ...
  def handle_escape_wait_15(text, pos)
    wait(15)
  end
  def handle_escape_wait_60(text, pos)
    wait(60)
  end
  ...
  def handle_escape_deactivate_show_fast(text, pos)
    @line_show_fast = false
    eagle_process_draw_update  # 因为之前在快进，此处更新一次排版
  end
  def handle_escape_skip_pause(text, pos)
    @pause_skip = true
  end
  def handle_escape_change_color(text, pos)
    font_params[:c] = obtain_escape_param_string(text)
    change_color(text_color(font_params[:c]))
  end
  def handle_escpae_draw_pic(text, pos)
    process_draw_pic(text, pos)
  end
  def handle_escpae_clc(text, pos)
  ...
  def eagle_win_update
    params = win_params
    self.x = params[:x] || default_init_x
    self.y = params[:y] || default_init_y
    MESSAGE_EX.reset_xy_dorigin(self, nil, params[:do]) if params[:do] < 0
  ...
  # 获取在地图上的pop对象（Game_Character的实例）
  ...
  # 获取在战斗中的pop对象（Sprite_Battler的实例）
  ...
  def eagle_pop_update
    params = pop_params
    eagle_pop_init_xy
    o = params[:o] # 设置对话框的显示原点
    o ||= 10 - params[:do]  # 显示原点恰好与绑定对象的位置相反
  ...
  def eagle_text_control_face(param = "")
    ...
    reset_eagle_sprite_face
    @eagle_sprite_face.apply_params if @eagle_sprite_face
  end
  ...
  def eagle_text_control_faceb(param = "")
    ...
    parse_param(face_params[:blink], param, :i)
  end
  ...
  # 文字绘制结束，等待按键前的处理
  def eagle_process_input_before
...
class Sprite_EaglePauseTag < Sprite
  ...
  #--------------------------------------------------------------------------
  def reset
    reset_source if @last_pause_index != params[:id] or params[:id] == 0 
    reset_bitmap
  end
...
module MESSAGE_EX
  ...
  def self.charapool_new(window, font, viewport)
    s = pool_new(:chara)
    return Sprite_EagleCharacter.new(window, font, viewport) if s.nil?
...
#=============================================================================
# ○ 将精灵池的初始化、更新、释放绑定到 Scene_Base 中
#=============================================================================
class Scene_Base
...
class Font_EagleCharacter
  attr_reader   :text, :icon # 绘制的文本/图标
  attr_reader   :width, :height  # 绘制的文本的宽高
  attr_reader   :dx, :dy  # 在绑定精灵的位图上绘制时的坐标偏移量
  # 获取文字颜色
  ...
  # 绘制叠加文字
  def draw_param_m(bitmap, x, y, w, h, dx=0, dy=0)
    color = bitmap.font.color.dup
    ...
    @params[:m].each do |c|
      if c.is_a?(Integer)
        ...
        bitmap.draw_text(x+dx, y+dy, w, h, c, 1)
      end
    end
    bitmap.font.color = color
  end
  ...
  # ● 先绑定精灵，并生成位图，才能进行绘制
  #--------------------------------------------------------------------------
  def bind(sprite, bitmap_width, bitmap_height, text_width, text_height)
    @sprite = sprite
    if sprite.bitmap and sprite.bitmap.width == bitmap_width and 
       sprite.bitmap.height == bitmap_height
      sprite.bitmap.clear
    else
      sprite.bitmap = Bitmap.new(bitmap_width, bitmap_height)
    end
    # 保存即将绘制的文字的宽高
    @width = text_width
    @height = text_height
    # 默认在 w*h 的范围内居中绘制文字
    @dx = (bitmap_width - @width) / 2
    @dy = (bitmap_height - @height) / 2
  end
  ...
  def draw(c, bitmap=@sprite.bitmap)
    ...
    if defined?(Sion_GradientText) && @params[:ex_cg] && @params[:ex_cg] != ''
      ...
      Sion_GradientText.draw_text(bitmap,x+@dx,y+@dy,w*2,@height,c,0,grad_cs)
    else
      # 奇怪的bug：绘制高度大于文字高度时，哪怕是左对齐，在RGD里也会垂直居中？
      bitmap.draw_text(x+@dx, y+@dy, w*2, @height, c, 0)
    end
  ...
  # 绘制单个图标
  def draw_icon(icon_index, bitmap = @sprite.bitmap)
    @icon = icon_index
    x = y = 0; w = bitmap.width; h = bitmap.height
  ...
  # 绘制单张图片
  def draw_pic(pic_bitmap, h, bitmap = @sprite.bitmap)
...
class Sprite_EagleCharacter < Sprite
  ...
  # 获取文字的实际矩形
  def real_rect
    Rect.new(@eagle_font.dx,@eagle_font.dy,@eagle_font.width,@eagle_font.height)
  end
  ...
  #--------------------------------------------------------------------------
  def initialize(window_bind, font_bind, viewport = nil)
    super(viewport)
  ...
  # 临时解绑视图
  def unbind_viewport; self.viewport = nil; end
  # 绑定回之前的视图
  def rebind_viewport; self.viewport = @viewport_bind; end
  ...
  # 文字左对齐时，相对对话框左上角的显示位置（方便后续自动对齐）
  def reset_origin_xy(x = nil, y = nil, flag = true)
    @origin_x = x if x
    @origin_y = y if y
    reset_xy(x, y) if flag
  end
  ...
  def reset_window_oxy(ox = nil, oy = nil)
    # (_ox,_oy) 是当前文字显示区域的显示原点位置（对话框内部坐标系）
    # （移入移出时解绑了视图，但如果对话框的文字显示区域的原点又发生变动，用此方法更新）
    @_ox = ox if ox
    @_oy = oy if oy
  end
  ...
  def update
    ...
    return move_update(@flag_move) if move_updating?
    if update_effects?
      super
      update_effects
    end
  ...
  # 正在移动？
  def move_updating?; @flag_move != nil; end

  # 更新移动
  def move_update(sym = :cin) # 只有移动结束时，才进行其他特效的更新
  ...
  def start_effect(sym, param_s)
    ...
    # 将调用对应特效的 start_effect_sym(param) 方法，其中 sym 为特效的转义符
  end
