...
#  ·窗口大小相关
     ...
     :hmax => 0,       # 窗口内容宽度的最大值（启用dh/fh时才生效）
                       #  * 值 < 行高时，将作为行数乘以行高
     :cdl  => 0,       # 文本左边距（额外增加的空白区域）（兼容 旧版本为:cdx）
     :cdu  => 0,       # 文本上边距（额外增加的空白区域）（兼容 旧版本为:cdy）
     :cdr  => 0,       # 文本右边距（额外增加的空白区域）（兼容 旧版本为:cdw）
     :cdd  => 0,       # 文本下边距（额外增加的空白区域）（兼容 旧版本为:cdh）
#  ·文本显示相关
     :se   => 0,       # 打字音类型序号（见下方[预设]）
     :ali  => 0,       # 文本对齐方式（0左对齐，1居中对齐，2右对齐）
     :ck   => 0,       # 文字间距
     :lh   => nil,     # 行高。若为 nil（$），则取当前字号
     :lhnl => 0,       # 换行时的行高（0取lh值，1取该行文字的最大高度）
     :ld   => 4,       # 行间距
...
#     ---------------------------------------------------------------------
#  ◇ 是否允许脸图增加窗口宽度（可能导致对话框实际宽度大于 :w 的预设）

   FACE_ADD_WINDOW_WIDTH = true

#     ---------------------------------------------------------------------
#  ◇ 是否允许姓名框增加窗口宽度和高度（可能导致对话框实际宽高大于 :w 和 :h 的预设）

   NAME_ADD_WINDOW_WIDTH_HEIGHT = true
...
#  ● \face[name|param]   - 文字绘制期间，脸图的动态设置

#  ● \faceb[name|param]  - 文字绘制完成后，脸图的动态设置
...
#     i   => n ,       #【重置】【快捷】脸图索引序号变更
#     l   => n ,       #【重置】文字绘制期间脸图循环播放，将从当前帧开始往后播放n-1帧，
#                          随后再从当前帧开始循环（默认 -1 不启用）
     :lt  => 20,       # 文字绘制期间脸图循环播放时，每两帧之间的等待帧数
     :lw  => 60,       # 文字绘制期间脸图循环播放时，循环结束时的等待帧数，nil（$）时不循环
     
#  ● \faceb[name|param]   ▼ 文字绘制结束后，脸图的动态设置

     :blink => {
  #     name=> str,      #【重置】脸图文件名称变更
  #                      # * 如果没有写 | 符号，则认定为不更改所用的脸图文件
  #     i   => n ,       #【快捷】脸图索引序号
  #     l   => n ,       #【重置】文字绘制结束后脸图循环播放，将从当前帧开始往后播放n-1帧，
  #                          随后再从当前帧开始循环（默认 -1 不启用）
       :lt  => 20,       # 文字绘制结束后脸图循环播放时，每两帧之间的等待帧数
       :lw  => 60,       # 文字绘制结束后脸图循环播放时，循环结束时的等待帧数，nil（$）时不循环
     },
...
#  ├ 旧脸图的移出方式（"hide"-直接隐藏  "out"-淡出） 
   FACE_OLD_OUT = "out"
#  ├ 新脸图的移入方式（"show"-直接显示  "in"-淡入） 
   FACE_NEW_IN  = "in"
...
#                jump  → 脸图跳跃一次（无可设定参数）
#                nod   → 脸图点头一次（仅可设置 t → 持续时间 和 dy → 最大向下距离）
...
#  ？ 示例
#     编写对话：文字变成\cg[c1c2c1]渐变色\cg[0]啦！
