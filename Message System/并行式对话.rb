...
# 【介绍】
#
# - 在默认的系统中，由于所有事件共用了一个全局的对话框窗口，
#   当对话框开启时，并行事件中会因为对话指令而暂停更新，
#   此外，共用的对话框在开启时，势必会挂起玩家的移动，导致无法制作更具动态的对话。
#
# - 本插件引入了全新绘制的在精灵Sprite上的对话框，
#   对于单个事件页，该对话框关闭后，才会继续执行之后的事件指令；
#   对于不同事件，该对话框能同时显示多个，且互不影响。
#
#------------------------------------------------------------------------------
# 【使用 - 事件的显示文字】
#
#  - 在 事件指令-注释 中填写 <并行对话> ，之后的【显示文字】指令将使用并行对话框；
#    而填写 <正常对话> ，则之后的【显示文字】将仍然使用默认的对话框。
#
# - 关于【显示文字】：
#
...
#
#  3. 在“文本框”中，编写 <wait 数字> 用于设置对话框的完全显示时间
#      为对话框移入移出中间的显示等待时间，不包括移入移出占用的时间
#
#  4. 在“文本框”中，编写 <io 模式 时间> 用于修改当前对话框的移入移出方式
#      模式 可以填入以下类型：
#         fade 代表淡入、淡出
#         zoom 代表放大移入、缩小移出
#      时间 为移入/移出的所用帧数
...
#
#  5. 在“文本框”中，编写 <skin id> 用于为当前对话框切换窗口皮肤
...
# 【注意】
#
# - 由于并行对话一般为自动显示自动隐藏，因此不再响应玩家按键，也不会阻碍玩家移动
#
# - 但如果玩家主动触发的事件中，存在并行对话，仍然需要等待事件结束，玩家才能继续移动
#
