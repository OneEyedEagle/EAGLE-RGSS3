...
#==============================================================================
$imported ||= {}
$imported["EAGLE-MessageChat"] = "1.2.2"
#==============================================================================
# - 2025.11.12.22 更新注释；在选择时禁止了聊天对话移动
#==============================================================================
...
#
#---------------------------------------------------------------------------
# - 在 事件指令-显示文字 中，新增以下规则：
#
#  （为了方便编写，标签中的英语冒号均可省略不写。）
#
#  1. 对于编辑器中的 “显示位置” ，（默认“居下”，即左对齐）
#
#      “居上” = 右对齐，“居中” = 居中对齐，“居下” = 左对齐。
#
#  2. 在 “角色肖像图” 中，对脸图的规格进行了扩展（同【鹰式对话框扩展】）
#
...
#
#     但为了方便排版，最终都会缩放到 FACE_W * FACE_H 的大小进行显示，
#     因此请确保脸图大小为该大小的整数倍，保证脸图不会被拉伸而模糊。
#
#  3. 在 “文字” 中，开头处编写 【...】 或 [...] 用于指定姓名。
#
#      如： 【小明】今天天气真好
#      将显示姓名：小明；显示文本：今天天气真好
#
#  4. 在 “文字” 中，编写 <pic: filename> 用于显示图片。
#     （图片需放置于 Graphics/Pictures 目录下。）
#
#      如： 【小明】<pic bird_happy>
#      将显示姓名：小明；显示图片：bird_happy.png
#
#     注意：图片将与其他文字拆分，单独作为一个对话框显示。
#     （图片若大于聊天框的宽度，将等比例缩小确保完整显示。）
#
#  5. 在 “文字” 中，编写 <no wait> 用于不等待按键，自动继续之后的指令。
#     （也可以编写 <nowait>，且字母大小写均可。）
#
#  6. 在 “文字” 中，编写 <skin: id> 用于为当前对话框切换窗口皮肤。
#     其中 id 会先在 ID_TO_SKIN 常量中查找预设的名称映射，
#       若不存在，则在 Graphics/System 目录下查找是否有该文件名的窗口皮肤。
#     如：<skin0> 将切换回默认皮肤
#     如：<skin Window_Blue> 将切换成 Window_Blue 名称的窗口皮肤文件
#
#  7. 在 “文字” 中，编写 <unsend: 数字> 用于在指定帧后撤回该对话（从开始显示计时）
#
#     特别的，可以编写数学表达式，其中可用 s 代表开关组，v 代表变量组。
#
...
#
#    对 string 的解析：执行eval(string)后返回布尔值，
...
#    示例：
#
#      这是第rb{v[1]}个选项  → 显示的选项内容为：这是第0个选项
#
#  3. 相邻且同级的 显示选项 指令将自动合并。
#
#    若不想自动合并，可在之间插入 注释 指令。
#
#    合并后只保留最后一个选项指令中的取消分支（其他选项指令的取消分支无效）。
#
#---------------------------------------------------------------------------
# 【注意】
#
...
class UI_EagleMessage_Chat
  ...
  def initialize
    init_sprite_hint
    @flag_no_move = false # 如果 true 则停止移动
    @blocks = [] # 新的被push到最后
...
class Sprite_EagleChat_Choice < Sprite_EagleChat
  ...
  def redraw_content_wh_result
    ...
    if params[:choice_result] == $game_message.eagle_chat_choice_cancel_i_e &&
      ...
      params[:choice_result_text] = params[:choices_draw][params[:choice_result]]
    end
    ps = { :font_size => MESSAGE_CHAT::FONT_SIZE, :x0 => 0, :y0 => 0,
      :w => params[:cont_w_max], :lhd => 2 }
