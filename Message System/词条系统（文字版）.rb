...
#==============================================================================
# - 2021.12.3.11 新增全局脚本判定某个词条的解锁情况
#==============================================================================
...
#   第二种格式： 词条|词条名称|信息词
#
#     其中 词条 为固定的识别文本，需要位于行首，前面不可有其它文本或空格。
#
#     其中 词条名称 为预设的名称，注意不同词条之间不相同。
#
#     其中不同 信息词 之间用空格隔开，若没有信息词，依然需要写两个 | 符号。
#
# - 为了方便词条文本的更新，本插件引入了词条的信息词，
#     在解锁词条时，能选择同时解锁其信息词。
...
# 【高级】
#
# - 利用全局脚本 DICT.unlock?(name) 来检查某个词条是否解锁
#
#   比如 DICT.unlock?("主角1") 返回词条 "主角1" 的解锁情况，
#     如果已经解锁，则为 true，否则为 false
#
# - 利用全局脚本 DICT.unlock?(name, sym) 来检查某个词条的某个信息词是否解锁
#
#   比如 DICT.unlock?("主角1", "基本信息") 为词条 "主角1" 的 "基本信息" 的解锁情况，
#     如果词条 "主角1" 已经解锁，且 "基本信息" 也解锁，则为 true，
#     如果词条 "主角1" 未解锁，则为 false，如果 "基本信息" 未解锁，也为 false
#
#---------------------------------------------------------------------------
# 【联动】
#
...
#
# - 当使用了【AddOn-关键词信息查看 by老鹰】，
#   其中的关键词解锁将同时解锁本插件的对应词条。
#
#===============================================================================
...
module DICT
  ...
  # ● 指定词条已经解锁？
  #--------------------------------------------------------------------------
  def self.unlock?(name, sym = nil)
    f = $game_system.eagle_dict[name]
    return false if f == nil
    if sym
      return f.include?(sym)
    else
      return true
    end
  end
...
class Game_Interpreter
  ...
  def command_108
    ...
    @comments.each do |t|
      ...
      if t =~ DICT::COMMENT_UNLOCK2
        k = $1
        v = $2.split(' ') rescue nil
        DICT.add(k, v)
      end
    end
  end
