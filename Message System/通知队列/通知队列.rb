#==============================================================================
# ■ 通知队列 by 老鹰（https://github.com/OneEyedEagle/EAGLE-RGSS3）
# ※ 本插件需要放置在【组件-通用方法汇总 by老鹰】与
#   【组件-位图绘制转义符文本 by老鹰】之下
#==============================================================================
$imported ||= {}
$imported["EAGLE-MessageHint"] = "1.1.4"
#==============================================================================
# - 2026.7.6.23 可以设置渐隐渐显的不透明度变化值了
#==============================================================================
...
module MESSAGE_HINT
  ...
  PARAMS["default"] = {
    ...
    #----------------------------------------------------------------------
    # - 等待该帧数后，自动移出最早的一条通知
    #  （如果设置为 0，则不会因为显示时间长而自动移出）
    "COUNT_MOVE_OUT" => 90,
    #----------------------------------------------------------------------
    # - 通知数量大于该值时，自动移出最早的一条通知
    #  （如果设置为 0，则不会因为数量多而自动移出）
    "COUNT_MAX_OUT" => 10,
    #----------------------------------------------------------------------
    # - 在移入移出过程中，需要消耗的帧数
    "HINT_IN_OUT_T" => 20,
    ...
    "HINT_DY_IN" => 0,
    #----------------------------------------------------------------------
    # - 在移入过程中，通知精灵的不透明度增加量（不透明度每帧增加该值，直至255）
    "HINT_OPA_IN" => 15,
    #----------------------------------------------------------------------
    ...
    "HINT_DY_OUT" => 0,
    #----------------------------------------------------------------------
    # - 在移出过程中，通知精灵的不透明度减少量（不透明度每帧减少该值，直至0）
    "HINT_OPA_OUT" => 15,
  }
...
#===============================================================================
class << Graphics
  alias eagle_message_hint_update update
  def update
    eagle_message_hint_update
    MESSAGE_HINT.update
  end
end
#===============================================================================
...
#===============================================================================
class << DataManager
  alias eagle_message_hint_init init
  def init
    eagle_message_hint_init
    MESSAGE_HINT.init
  end
end
#===============================================================================
...
class Spriteset_EagleHint
  ...
  def update_out_count
    ...
    @count += 1
    v1 = @params["COUNT_MOVE_OUT"]
    v2 = @params["COUNT_MAX_OUT"]
    if (v1 > 0 && @count > v1) ||
       (v2 > 0 && @sprites.size > v2 && !@sprites[0].in_or_out?)
      @num_out += 1
      @count = 0
      reset_params
    end
...
class Sprite_EagleHint < Sprite
  ...
  def update_in_out
    if @params_move[:in]
      if self.opacity < 255
        v = @spriteset.params["HINT_OPA_IN"].to_i rescue 15
        self.opacity += v
      else
        @params_move[:in] = false
      end
    ...
    if @params_move[:out]
      if self.opacity > 0
        v = @spriteset.params["HINT_OPA_OUT"].to_i rescue 15
        self.opacity -= v
      else
        @params_move[:out] = false
      end
