...
#
# - 如何设置一个 “对话标签对”：
#   利用下列格式进行编写，其中...替换成事件编辑器中显示文字时所写的文本内容
...
#
# - 如何快捷设置一组 “对话标签对”：
#   在<list>...</list>标签对中，利用 <call name> 可直接获取并增加预设的name名称的
#   并行对话序列中的全部内容
#    示例：<list><call test1><msg>...</msg></list>
#        → 使用预设的test1名称的并行对话序列中的全部对话
#         （需成功判定test1的对应cond不为false，否则不会增加），并在最后加上一个msg
#
# - 注意：
#    ·所有标签对内容均大小写敏感，在预设情况下，全部都为小写字母
#    ·进行 场所移动 / Scene切换 时，会自动结束全部并行对话序列
#    ·打开所设定的开关时，会自动结束全部并行对话序列并禁止产生新的并行对话
#
#--------------------------------------------------------------------------
# - 地图事件
