#==============================================================================
# ■ UI移动控制 by 老鹰（https://github.com/OneEyedEagle/EAGLE-RGSS3）
#==============================================================================
$imported ||= {}
$imported["EAGLE-UIMove"] = "1.0.1"
#=============================================================================
# - 2022.8.21.22 修复 ulm 报错的bug
#=============================================================================
# - 本插件提供了对窗口/精灵的移动控制
#-----------------------------------------------------------------------------
...
class Eagle_MoveControl
  ...
  def command_ulm(param)
    ps = param.split(',')
    @ex[:ulm] = {}
    @ex[:ulm][:x] = ps[0].to_i if ps[0] != ""
    @ex[:ulm][:y] = ps[1].to_i if ps[1] != ""
  end
  #--------------------------------------------------------------------------
  # ● 处理移动（当 @t 大于 0 时）
  #--------------------------------------------------------------------------
  ...
  def process_before_move
    if @ex[:ulm]
      @ex[:ulm][:vx] = (@ex[:ulm][:x] - @x) * 1.0 / @t
      @ex[:ulm][:vy] = (@ex[:ulm][:y] - @y) * 1.0 / @t
    end
  end
  #--------------------------------------------------------------------------
  # ● 移动时处理1
  ...
  #--------------------------------------------------------------------------
  def process_move_xy
    return process_move_ulm if @ex[:ulm]
    @x += @vx; @y += @vy
    @vx += @ax; @vy += @ay
  end
  #--------------------------------------------------------------------------
