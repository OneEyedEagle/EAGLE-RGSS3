#==============================================================================
# ■ 按键输入扩展 by 老鹰（http://oneeyedeagle.lofter.com/）
#==============================================================================
$imported ||= {}
...
#-----------------------------------------------------------------------------
# - 检测指定按键是否从 未按下 到 按下
#      Input_EX.trigger?(key)
#   其中 key 为自定义的按键标识符，见 KEY_LIST 哈希表的键值
#   如： Input_EX.trigger?(:SPACE) 返回空格键是否在当前帧从未按下到按下状态
#
# - 检测指定按键是否从 按下 到 未按下
#      Input_EX.up?(key)
#
...
#      Input_EX.press?(key)
#   获取 按住 的持续帧数
#      Input_EX.press_count(key)
#
# - 检测一组按键是否被 按下
#      Input_EX.trigger_m?(keys)
#   其中 keys 数组为 KEY_M 中含有的按键的组合
#=============================================================================
module Input_EX
  #--------------------------------------------------------------------------
  # ● 【常量】定义按键标识符
  ...
  # ● 获取一组按键的状态序列数字
  #--------------------------------------------------------------------------
  def self.get_keys_v(keys)
    _v = "0" * KEY_M.size
    KEY_M.each_with_index do |key, i|
      _v[i] = (keys.include?(key) ? "1" : "0")
    end
    _v.to_i(2)
  end
  #--------------------------------------------------------------------------
  # ● 一组按键被按下？
  #--------------------------------------------------------------------------
  def self.trigger_m?(keys)
    @keys_m_v.to_i(2) == get_keys_v(keys)
  end
end
class << Input
  #--------------------------------------------------------------------------
