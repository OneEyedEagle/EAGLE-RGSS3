#==============================================================================
# ■ Add-On 文字四散移出 by 老鹰（https://github.com/OneEyedEagle/EAGLE-RGSS3）
# ※ 本插件需要放置在【粒子模板-发射类 by老鹰】之下
#==============================================================================
$imported ||= {}
$imported["EAGLE-Particle-CharasSpread"] = "3.0.0"
#==============================================================================
...
module PARTICLE
  ...
  def self.pout(sym, charas, params)
    if PARTICLE.templates[sym]     # 设置显示端口
      vp = PARTICLE.templates[sym].viewport
    else
      vp = Viewport.new
    end
    vp.z = 1000
    vp.add_fast_layer(1, 0) if $RGD # 整合RGD加速
    if PARTICLE.templates[sym]
      f = PARTICLE.templates[sym]
      f.viewport = vp
    else
      f = PT_Emitter.new(vp)
    end
