...
class << PARTICLE
  ...
  # ○ 绑定
  #--------------------------------------------------------------------------
  alias eagle_particle_ui_bglines_init init
  def init
    eagle_particle_ui_bglines_init

    # 预设模板
    #  全屏随机显示细小方块，并且相互连线
    f = PT_Emitter_Rebound_DrawLines.new
    ps = PARTICLE::TEMPLATE_UI_BG_PARAMS
    b = Bitmap.new(ps[:bitmap_wh], ps[:bitmap_wh])

    # 绘制点
    b.fill_rect(b.rect, ps[:color])
    # 传入位图，粒子会随机选一个
...
# ■ 粒子模板类 - 连线
#==============================================================================
class PT_Emitter_Rebound_DrawLines < PT_Emitter_ReboundBox
  def init_settings
    super
