...
#==============================================================================
$imported ||= {}
$imported["EAGLE-MouseEX"] = "1.2.6"
#=============================================================================
# - 2026.6.28.12 修复精灵改变src_rect后依然能判定为鼠标选中的bug
#=============================================================================
# - 本插件新增了一系列鼠标控制的方法
...
class Sprite
  ...
  def mouse_in?(_visible = true, _pixel = true)
    ...
    if _visible
      return false if self.visible == false || self.opacity == 0
      if self.bitmap && (self.src_rect.y >= self.bitmap.height ||
         self.src_rect.y <= -self.bitmap.height || 
         self.src_rect.x >= self.bitmap.width ||
         self.src_rect.x <= -self.bitmap.width)
        return false 
      end
    end
